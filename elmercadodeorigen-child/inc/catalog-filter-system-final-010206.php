<?php
/**
 * Sistema definitivo de filtros de catálogo 0.10.206.
 *
 * Un único módulo controla contexto de categoría, vendedor, filtros aplicados,
 * orden y presentación. El generador de atributos específicos permanece como
 * módulo funcional sin CSS ni JavaScript. Este archivo no usa MutationObserver,
 * temporizadores ni estilos calculados en runtime.
 *
...
/**
 * Devuelve IDs de categoría que forman el contexto actual.
 *
 * @return int[]
 */
function elmercado_catalog_context_term_ids_010206(): array {
...
/**
 * Cuenta productos publicados por autor con una única consulta.
 *
 * @return array<int,int>
 */
function elmercado_catalog_vendor_counts_010206(): array {
	global $wpdb;

	$term_ids = elmercado_catalog_context_term_ids_010206();
	if ( $term_ids ) {
		...
		$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$wpdb->prepare(
				"SELECT post_author, COUNT(ID) AS product_count
				FROM {$wpdb->posts}
...
function elmercado_core_filter_vendors(): array {
	$vendors = array();
	foreach ( array_keys( elmercado_catalog_vendor_counts_010206() ) as $vendor_id ) {
	...
	if ( $profile ) {
		foreach ( (array) ( $profile['attributes'] ?? array() ) as $slug => $label ) {
			...
			foreach ( $terms as $term_slug ) {
				...
				$remove    = $remaining
					? add_query_arg( $filter_param, implode( ',', $remaining ), $current )
					: remove_query_arg( array( $filter_param, $query_param ), $current );
				$chips[]   = array(
					'group' => (string) $label,
					'name'  => $term instanceof WP_Term ? (string) $term->name : $term_slug,
					'url'   => $remove,
				);
			}
		}
...
/**
 * Markup pendiente de montaje. Se mantiene oculto hasta que un único script lo
 * inserta en el sidebar para que nunca aparezca en una posición intermedia.
 */
add_action(
	...
	static function (): void {
		...
		$counts    = elmercado_catalog_vendor_counts_010206();
...
add_action(
	...
	static function (): void {
		...
		?>
		<style id="elmercado-catalog-filter-system-final-010206">
			/* Home: contador como señal de navegación; ninguna flecha decorativa. */
			...
			/* Los elementos pendientes nunca se ven fuera del sidebar. */
			#emo-category-context[hidden], #emo-global-vendor-filter[hidden], #emo-category-attribute-filters[hidden] { display:none !important; }

			/* La clase de montaje es la única autoridad de visibilidad final. */
			html body.elmercado-child-theme:is(.woocommerce-shop,.tax-product_cat) :is(#secondary.widget-area,.shop-widget-area,.emo-mobile-filter-content .widget-area) > #emo-category-context.emo-catalog-filter-mounted,
			html body.elmercado-child-theme:is(.woocommerce-shop,.tax-product_cat) :is(#secondary.widget-area,.shop-widget-area,.emo-mobile-filter-content .widget-area) > #emo-global-vendor-filter.emo-catalog-filter-mounted,
			html body.elmercado-child-theme:is(.woocommerce-shop,.tax-product_cat) :is(#secondary.widget-area,.shop-widget-area,.emo-mobile-filter-content .widget-area) > #emo-category-attribute-filters.emo-catalog-filter-mounted {
			...
			@media (min-width:1101px) {
				body.elmercado-child-theme:is(.woocommerce-shop,.tax-product_cat) :is(#secondary.widget-area,.shop-widget-area) {
					box-sizing:border-box !important; width:250px !important; min-width:250px !important; max-width:250px !important; padding:18px !important;
					border:1px solid rgba(23,63,50,.11) !important; border-radius:18px !important; background:#fff !important; box-shadow:0 12px 32px rgba(17,42,34,.07) !important;
				}
			}

			/* En categoría la propia tarjeta de contexto sustituye al árbol de categorías. */
			...
			body.elmercado-child-theme.tax-product_cat #emo-category-context { box-sizing:border-box !important; width:100% !important; margin:0 0 14px !important; padding:0 !important; border:0 !important; border-radius:0 !important; background:transparent !important; box-shadow:none !important; }
			...
			body.elmercado-child-theme.tax-product_cat #emo-category-context .emo-category-context__remove:focus-visible { color:#173f32 !important; text-decoration:underline !important; text-underline-offset:3px !important; }

			/* En Tienda solo quedan los tres filtros base. */
			...
			/* Únicamente Limpiar todo: no existe una segunda acción del panel específico. */
			...
			/* Titulares: texto + regla fina; nunca tarjeta. */
			...
			/* No hay separadores entre grupos; la jerarquía la da el espacio. */
			body.elmercado-child-theme.tax-product_cat #emo-category-attribute-filters .emo-category-filter-group { border-bottom-width:0 !important; }

			/* Gap real: dos seleccionados consecutivos nunca forman una sola mancha. */
			...
			/* Hover y seleccionado son idénticos; el peso nunca cambia y el texto no baila. */
			...
			body.elmercado-child-theme:is(.woocommerce-shop,.tax-product_cat) :is(#secondary.widget-area,.shop-widget-area,.emo-mobile-filter-content .widget-area) :is(.widget_product_categories li:hover > .count,.widget_product_categories .current-cat > .count,#emo-global-vendor-filter .emo-global-vendor-filter__item:hover > .count,#emo-global-vendor-filter .emo-global-vendor-filter__item.is-active > .count,#emo-category-attribute-filters .woocommerce-widget-layered-nav-list__item:hover > .count,#emo-category-attribute-filters .woocommerce-widget-layered-nav-list__item.chosen > .count,#emo-category-attribute-filters .woocommerce-widget-layered-nav-list__item--chosen > .count) { color:#155b42 !important; }

			/* Sin pseudo-checkboxes ni flechas heredadas. */
			...
			body.elmercado-child-theme:is(.woocommerce-shop,.tax-product_cat) :is(.widget_product_categories li,#emo-global-vendor-filter .emo-global-vendor-filter__item) > :is(svg,i,.arrow,.caret,.chevron,.woostify-svg-icon,.cat-toggle,.category-toggle) { display:none !important; }

			/* Filtros aplicados: un único bloque y una única acción de limpieza. */
...
/**
 * Montaje estructural único. Solo mueve nodos y añade una clase de estado;
 * nunca escribe propiedades visuales ni vuelve a ejecutarse más tarde.
 */
add_action(
	'wp_footer',
	static function (): void {
		...
		?>
		<script id="elmercado-catalog-filter-controller-final-010206">
		(() => {
			'use strict';
			...
			/* Solo normalización textual; no hay estilo en runtime. */
