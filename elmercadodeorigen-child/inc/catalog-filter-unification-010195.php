<?php
/**
 * Sistema visual unificado de filtros 0.10.195.
 *
...
/**
 * Cuenta productos publicados de un vendedor en el contexto actual.
 */
function elmercado_filter_vendor_count_010195( int $vendor_id ): int {
	$args = array(
		'post_type'              => 'product',
		'post_status'            => 'publish',
		'author'                 => $vendor_id,
...
/**
 * Datos que el controlador usa para completar los contadores visuales.
 */
function elmercado_filter_counts_010195(): array {
	$data = array(
		'vendors'    => array(),
		'categories' => array(),
	);
	...
	if ( function_exists( 'elmercado_core_filter_vendors' ) ) {
		foreach ( elmercado_core_filter_vendors() as $vendor_id => $vendor_name ) {
			$count = elmercado_filter_vendor_count_010195( (int) $vendor_id );
			if ( $count > 0 ) {
...
add_action(
	...
	static function (): void {
		...
		?>
		<style id="elmercado-catalog-filter-unification-010195">
			/* Home: contenido de categoría más compacto y el dato sustituye a la flecha. */
			...
			body.home.elmercado-child-theme .emo-category-card > svg { display:none !important; }

			/* Todos los filtros comparten caja, ritmo y separador. */
			...
			/* Titulares: resaltado suave para separar visualmente cada refinamiento. */
			...
			/* Categorías y Vendedor replican las filas de los filtros específicos. */
			...
			body.elmercado-child-theme:is(.woocommerce-shop,.tax-product_cat) :is(#secondary.widget-area,.shop-widget-area) :is(.widget_product_categories .current-cat,#emo-global-vendor-filter .emo-global-vendor-filter__item.is-active,#emo-category-attribute-filters .woocommerce-widget-layered-nav-list__item.chosen) {
				background:#edf4ef !important;
				box-shadow:inset 3px 0 0 #2f7d5d !important;
			}

			/* Los grupos específicos mantienen el mismo ritmo que los filtros base. */
			...
			/* Contexto de categoría: siempre primero y con salida clara a Tienda. */
			...
			body.elmercado-child-theme.tax-product_cat #emo-category-applied-filters-slot-010195 { width:100% !important; margin:0 !important; padding:6px 0 0 !important; }
		</style>
		<?php
	},
	PHP_INT_MAX
...
add_action(
	...
	static function (): void {
		...
		$counts = elmercado_filter_counts_010195();
		?>
		<script id="elmercado-catalog-filter-unification-controller-010195">
		(() => {
			'use strict';
			...
			const cleanCount = (badge, value = null) => {
				...
				badge.textContent = value === null ? (badge.textContent || '').replace(/[()]/g, '').trim() : String(value);
				...
				if (isCategory) {
					if (active) {
						if (specific) {
							...
							let slot = document.getElementById('emo-category-applied-filters-slot-010195');
							if (!slot) { slot = document.createElement('div'); slot.id = 'emo-category-applied-filters-slot-010195'; }
							if (slot.parentElement !== sidebar) sidebar.append(slot);
							if (active.parentElement !== slot) slot.append(active);
						}
					}
					const afterVendor = specific || document.getElementById('emo-category-applied-filters-slot-010195');
			...
			const schedule = () => { if (!scheduled) { scheduled = true; requestAnimationFrame(place); } };
			place();
