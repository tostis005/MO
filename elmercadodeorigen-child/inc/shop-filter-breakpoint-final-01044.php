...
add_action(
	...
	static function (): void {
		...
		?>
		<script id="elmercado-shop-filter-controller-final-01044">
		(() => {
			'use strict';
			...
			const sidebar = document.querySelector('.emo-mobile-filter-content #secondary.widget-area,.emo-mobile-filter-content .widget-area,#secondary.widget-area,.shop-widget-area,.content-area + .widget-area');
			if (!toggle || !shell || !content || !sidebar) return;

			/* Retiramos exclusivamente el listener antiguo del trigger, cuya lógica terminaba en 991px. */
			...
			let close = shell.querySelector('.emo-mobile-filter-close');
			if (close) {
				const cleanClose = close.cloneNode(true);
				close.replaceWith(cleanClose);
				close = cleanClose;
			}
			...
			const moveIntoDrawer = () => {
				if (compact() && sidebar.parentElement !== content) content.append(sidebar);
			};
