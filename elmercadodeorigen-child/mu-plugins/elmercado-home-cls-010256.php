<?php
/**
 * CLS stabilization for the cached and dynamic Home first frame.
 *
 * The Home cache extracts wp_head <style> blocks to a deferred stylesheet and
 * keeps only `elmercado-home-first-view-css` in the critical path. Anonymous
 * WooCommerce sessions can bypass that HTML cache, so this MU plugin also runs
 * the same first-paint transformation on dynamic public Home responses without
 * caching or sharing their cart/session HTML.
 *
...
/**
 * Make dynamic anonymous Home responses use the same critical/deferred CSS path
 * as cacheable Home responses. The HTML itself remains uncached/personalized.
 */
function elmercado_home_prepare_dynamic_first_paint_010256( string $html ): string {
	if (
		str_contains( $html, 'id="elmercado-home-first-view-css"' )
		|| is_user_logged_in()
		|| ! function_exists( 'elmercado_optimize_home_document_for_first_paint' )
	) {
		return $html;
	}

	return elmercado_optimize_home_document_for_first_paint( $html );
}
...
function elmercado_home_patch_first_view_html_010256( string $html ): string {
	...
	$html = elmercado_home_prepare_dynamic_first_paint_010256( $html );

	if ( str_contains( $html, 'elmercado-home-first-view-lock-010256' ) ) {
		return $html;
	}
	 ...
	 * Persist the patched response again only when the request itself is safe to
	 * cache. Dynamic WooCommerce-session responses are deliberately not stored.
	 */
	if (
		$patched
