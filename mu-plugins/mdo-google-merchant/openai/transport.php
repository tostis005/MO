...
function mdo_openai_encrypt_secret_20260909( string $plain ): string {
    ...
    $iv = random_bytes( 12 );
    $tag = '';
    $encrypted = openssl_encrypt( $plain, $cipher, mdo_openai_secret_key_20260909(), OPENSSL_RAW_DATA, $iv, $tag, 'mdo-openai-commerce' );
...
final class MDO_OpenAI_Sftp_Transport_20260909 implements MDO_OpenAI_Transport_Interface_20260909 {
    ...
    private function phpseclib_connect() {
        ...
        try {
            ...
            return $ok ? $sftp : new WP_Error( 'sftp_auth', 'OpenAI SFTP rechazó la autenticación.' );
        } catch ( Throwable $e ) {
    ...
    private function ssh2_connect_auth() {
        ...
        try {
            if ( 'password' === $c['method'] ) {
                $ok = @ssh2_auth_password( $connection, $c['username'], $c['password'] );
        ...
        return $ok ? $connection : new WP_Error( 'sftp_auth', 'OpenAI SFTP rechazó la autenticación.' );
    }
    ...
    public function upload( string $local_path ) {
        ...
        if ( class_exists( '\\phpseclib3\\Net\\SFTP' ) ) {
            ...
            try {
                ...
                if ( ! $sftp->rename( $remote_tmp, $remote ) ) {
                    $sftp->delete( $remote_tmp );
                    return new WP_Error( 'sftp_rename', 'El archivo subió pero no pudo reemplazar atómicamente el snapshot remoto.' );
                }
                return array( 'ok' => true, 'remote' => $remote, 'bytes' => filesize( $local_path ) );
            } catch ( Throwable $e ) {
                return new WP_Error( 'sftp_exception', mdo_openai_clean_text_20260909( $e->getMessage(), 300 ) );
            }
        }
...
final class MDO_OpenAI_Api_Transport_20260909 implements MDO_OpenAI_Transport_Interface_20260909 {
    ...
    private function headers( bool $idempotent = false ): array {
        $headers = array(
    ...
    private function request( string $method, string $path, ?array $body = null ) {
        $base = untrailingslashit( esc_url_raw( (string) $this->settings['api_base_endpoint'] ) );
        if ( '' === $base ) { return new WP_Error( 'api_endpoint', 'Falta el base endpoint de Commerce proporcionado por OpenAI.' ); }
        $url = $base . '/' . ltrim( $path, '/' );
        $args = array( 'method' => $method, 'timeout' => 20, 'headers' => $this->headers( in_array( $method, array( 'POST','PATCH','PUT','DELETE' ), true ) ) );
        ...
        $safe = is_array( $decoded ) ? $decoded : array( 'body' => mdo_openai_clean_text_20260909( $raw, 1000 ) );
        if ( $code < 200 || $code >= 300 ) { return new WP_Error( 'api_http_' . $code, 'Commerce API respondió HTTP ' . $code . '.', $safe ); }
        return array( 'ok' => true, 'status' => $code, 'response' => $safe );
    }

    public function test_connection() {
        if ( ! $this->is_configured() ) { return new WP_Error( 'api_not_configured', 'Commerce API todavía no está configurada.' ); }
        return $this->request( 'GET', '/product_feeds/' . rawurlencode( (string) $this->settings['api_feed_id'] ) );
    }

    public function upload( string $local_path ) {
        if ( ! $this->is_configured() ) { return new WP_Error( 'api_not_configured', 'Commerce API todavía no está configurada.' ); }
        // The stable file-upload schema and the Commerce incremental Product API
        // are different contracts. Never POST JSONL rows into the Products API.
        $custom = apply_filters( 'mdo_openai_api_upload', null, $local_path, $this->settings, $this );
        if ( null !== $custom ) { return $custom; }
        return new WP_Error(
            'api_mapper_not_enabled',
            'La API incremental está preparada pero no activada: OpenAI debe proporcionar la configuración/credenciales y confirmar el contrato de Product API para esta cuenta. El snapshot se entrega por SFTP hasta entonces.'
        );
    }
}
