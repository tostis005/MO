...
/** Resolve the marker term ID without term-query recursion. */
function mdo_mentta_marker_term_id() {
	static $term_id = null;
	if ( null !== $term_id ) { return $term_id; }
...
/** Resolve MENTTA plus every descendant directly from taxonomy tables. */
function mdo_mentta_internal_term_ids() {
	static $ids = null;
	if ( null !== $ids ) { return $ids; }

	$root_id = mdo_mentta_marker_term_id();
	if ( ! $root_id ) { return $ids = array(); }
	...
	return $ids = array_values( array_unique( array_map( 'intval', $ids ) ) );
}

/** Resolve IDs/slugs/names/tt_ids for the internal tree. */
function mdo_mentta_internal_term_map() {
	static $map = null;
	if ( null !== $map ) { return $map; }

	$ids = mdo_mentta_internal_term_ids();
	$map = array( 'ids' => $ids, 'tt_ids' => array(), 'slugs' => array(), 'names' => array() );
	if ( ! $ids ) { return $map; }
...
/** Hide internally only on storefront requests, including frontend AJAX. */
function mdo_mentta_should_hide_publicly() {
	...
	if ( $doing_ajax ) {
		...
		if ( '' !== $path && false !== strpos( $path, '/wp-admin/' ) ) { return false; }
		return true;
	}
...
/** Hide MENTTA and descendants from public category lists/grids/widgets. */
function mdo_mentta_hide_from_public_term_queries( $args, $taxonomies ) {
	if ( ! mdo_mentta_should_hide_publicly() || ! in_array( 'product_cat', (array) $taxonomies, true ) ) { return $args; }
	$internal_ids = mdo_mentta_internal_term_ids();
	if ( ! $internal_ids ) { return $args; }
...
function mdo_mentta_hide_from_public_term_results( $terms, $taxonomies, $args, $term_query ) {
	if ( ! mdo_mentta_should_hide_publicly() || ! is_array( $terms ) ) { return $terms; }

	$requested_taxonomies = array_merge( (array) $taxonomies, isset( $args['taxonomy'] ) ? (array) $args['taxonomy'] : array() );
	if ( ! in_array( 'product_cat', $requested_taxonomies, true ) ) { return $terms; }

	$map = mdo_mentta_internal_term_map();
	if ( ! $map['ids'] ) { return $terms; }
	$fields = isset( $args['fields'] ) ? (string) $args['fields'] : 'all';

	return array_values( array_filter( $terms, static function ( $term ) use ( $map, $fields ) {
		if ( $term instanceof WP_Term ) { return ! in_array( (int) $term->term_id, $map['ids'], true ); }
		if ( 'ids' === $fields ) { return ! in_array( (int) $term, $map['ids'], true ); }
		if ( 'tt_ids' === $fields ) { return ! in_array( (int) $term, $map['tt_ids'], true ); }
		if ( 'slugs' === $fields ) { return ! in_array( strtolower( (string) $term ), $map['slugs'], true ); }
		if ( 'names' === $fields ) { return ! in_array( strtolower( trim( (string) $term ) ), $map['names'], true ); }
		return true;
	} ) );
}
...
function mdo_mentta_hide_from_public_product_terms( $terms, $post_id, $taxonomy ) {
	if ( 'product_cat' !== $taxonomy || ! mdo_mentta_should_hide_publicly() || ! is_array( $terms ) ) { return $terms; }
	$internal_ids = mdo_mentta_internal_term_ids();
	if ( ! $internal_ids ) { return $terms; }
	return array_values( array_filter( $terms, static function ( $term ) use ( $internal_ids ) {
...
/** Hide menu items pointing to the internal category tree. */
function mdo_mentta_hide_public_menu_items( $items ) {
	if ( ! mdo_mentta_should_hide_publicly() || ! is_array( $items ) ) { return $items; }
	$map = mdo_mentta_internal_term_map();

	return array_values( array_filter( $items, static function ( $item ) use ( $map ) {
		if ( isset( $item->object, $item->object_id ) && 'product_cat' === $item->object && in_array( (int) $item->object_id, $map['ids'], true ) ) { return false; }
		if ( isset( $item->url ) ) {
			$path = strtolower( (string) wp_parse_url( (string) $item->url, PHP_URL_PATH ) );
			foreach ( $map['slugs'] as $slug ) {
				if ( preg_match( '#/' . preg_quote( $slug, '#' ) . '/?$#i', untrailingslashit( $path ) . '/' ) ) { return false; }
			}
		}
		return true;
	} ) );
...
/** Home-card output hiding is handled by 000-mdo-mentta-home-final.php. */

/** Do not expose browsable public archives for MENTTA or descendants. */
function mdo_mentta_block_public_archive() {
	if ( ! mdo_mentta_should_hide_publicly() || ! function_exists( 'is_product_category' ) || ! is_product_category() ) { return; }
	$queried = get_queried_object();
	if ( ! ( $queried instanceof WP_Term ) || 'product_cat' !== $queried->taxonomy ) { return; }
	if ( ! in_array( (int) $queried->term_id, mdo_mentta_internal_term_ids(), true ) ) { return; }
	global $wp_query;
	$wp_query->set_404();
	status_header( 404 );
	nocache_headers();
