...
final class MDO_Connector_Huerta_Ana_Mary {
	...
	public static function scrape_product( string $url ): array {
		...
		$description = self::description( $xpath, $json, $title, $id );
	...
	private static function price_from_catalog( string $product_url ): ?float {
		...
		for ( $page = 1; $page <= 10; $page++ ) {
			...
			try {
				$xpath = self::xpath( self::fetch_html( $catalog_url ) );
			...
			foreach ( $xpath->query( '//a[@href]' ) ?: array() as $link ) {
				...
				if ( self::canonical_url( $href ) !== $target ) {
					continue;
				}
				...
				for ( $i = 0; $i < 7 && $node->parentNode instanceof DOMElement; $i++ ) {
					...
					$text = self::repair_text( (string) $node->textContent );
					if ( mb_strlen( $text ) > 2500 ) {
						break;
					}
					if ( preg_match_all( '/(\d{1,4}(?:[.,]\d{1,2})?)\s*€/u', $text, $matches ) ) {
						$values = array_values( array_unique( array_map( 'strval', $matches[1] ) ) );
						if ( 1 === count( $values ) ) {
							return self::parse_price( $values[0] );
						}
					}
				}
			}
	...
	private static function description( DOMXPath $xpath, array $json, string $title, string $id ): string {
