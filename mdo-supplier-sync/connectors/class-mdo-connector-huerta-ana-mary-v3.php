...
final class MDO_Connector_Huerta_Ana_Mary {
	...
	public static function discover( array $supplier ): array {
		$catalog   = self::canonical_url( (string) $supplier['source_url'] );
		$queue     = array( $catalog );
		$visited   = array();
		$products  = array();
		$excluded  = array();
		...
		while ( $queue && count( $visited ) < self::MAX_CATALOG_PAGES ) {
			$url = self::canonical_url( (string) array_shift( $queue ) );
	...
	public static function scrape_product( string $url ): array {
		...
		if ( ! $images ) {
			throw new RuntimeException( 'No se pudo detectar la imagen de producto en /data/productos/imagenes/.' );
		}
	...
	public static function upsert_product( int $supplier_id, array $product ): string {
		...
		// El ID final de la ficha es estable y evita duplicados si cambia el título o
		// si una versión antigua había guardado una URL relativa incorrectamente.
		...
		if ( $existing && 'excluded' === (string) $existing['status'] ) {
			$wpdb->update(
				$table,
	...
	private static function price_from_html( DOMXPath $xpath ): ?float {
		foreach ( array( "//*[@itemprop='price']/@content", "//meta[@property='product:price:amount']/@content", "//*[@data-price]/@data-price" ) as $query ) {
	...
	private static function description( DOMXPath $xpath, array $json, string $title ): string {
		if ( ! empty( $json['description'] ) ) {
			$clean = self::clean_description_text( (string) $json['description'], $title );
		...
		foreach ( array(
			...
			foreach ( $xpath->query( $query ) ?: array() as $node ) {
				$clean = self::clean_description_text( (string) $node->textContent, $title );
				if ( mb_strlen( $clean ) >= 40 ) {
					return wpautop( esc_html( $clean ) );
				}
			}
		...
		foreach ( $xpath->query( '//h1|//h2|//h3|//h4|//h5' ) ?: array() as $heading ) {
			...
			for ( $i = 0; $i < 8 && $container->parentNode instanceof DOMElement; $i++ ) {
				...
				if ( false !== mb_stripos( $text, 'Te interesa' ) || false !== mb_stripos( $text, 'Consejos de conservación' ) ) {
	...
	private static function clean_description_text( string $text, string $title ): string {
		$text = self::repair_text( html_entity_decode( wp_strip_all_tags( $text ), ENT_QUOTES | ENT_HTML5, 'UTF-8' ) );
		...
		foreach ( array( 'Te interesa', '€/kg', '€/caja', '€/ud', '€/unidad', 'Consejos de conservación:', 'Recetas relacionadas:', 'Seguir Comprando', 'Carrito de la Compra' ) as $stop ) {
			$pos = mb_stripos( $text, $stop );
			if ( false !== $pos && ( null === $cut || $pos < $cut ) ) {
				$cut = $pos;
			}
		}
		if ( null !== $cut ) {
	...
	private static function images( DOMXPath $xpath, array $json, string $base_url ): array {
		...
		// Las imágenes reales de producto están en /data/productos/imagenes/.
		// /data/documentos/imagenes/ son recetas/noticias y se excluyen expresamente.
		foreach ( $xpath->query( '//img[@src or @data-src or @data-original or @data-lazy-src or @srcset]' ) ?: array() as $img ) {
			if ( ! $img instanceof DOMElement ) {
				continue;
			}
			$raw = (string) ( $img->getAttribute( 'data-original' ) ?: $img->getAttribute( 'data-lazy-src' ) ?: $img->getAttribute( 'data-src' ) ?: $img->getAttribute( 'src' ) );
			if ( ! $raw && $img->hasAttribute( 'srcset' ) ) {
				$first = trim( explode( ',', (string) $img->getAttribute( 'srcset' ) )[0] );
				$raw   = trim( explode( ' ', $first )[0] );
			}
			if ( preg_match( '~(?:^|/)data/productos/imagenes/[^/?]+\.(?:jpe?g|png|webp|gif)(?:\?|$)~i', html_entity_decode( $raw, ENT_QUOTES | ENT_HTML5, 'UTF-8' ) ) ) {
				self::add_image( $images, $raw, $base_url );
			}
		}
	...
	private static function add_image( array &$images, string $raw, string $base_url ): void {
		...
		if ( $url && preg_match( '~\.(?:jpe?g|png|webp|gif)(?:\?|$)~i', $url ) && str_contains( (string) wp_parse_url( $url, PHP_URL_PATH ), '/data/productos/imagenes/' ) ) {
			$images[ $url ] = $url;
		}
	}
	...
	private static function clean_title( string $title, string $id, string $url ): string {
		// Regla fija solicitada: el producto 113 es siempre "Flores de calabacín 8 unidades".
		// El emparejamiento sigue dependiendo del ID 113, no del texto, para evitar duplicados.
		if ( '113' === $id ) {
	...
	private static function is_product_url( string $url, string $catalog ): bool {
		...
		return (bool) preg_match( '~-[0-9]+\.html$~i', $path )
			&& ! preg_match( '~/(?:noticias|recetas|blog)/~i', $path )
			&& str_contains( strtolower( $path ), '/hortalizas-y-conservas/' );
	}
	...
	private static function absolute_url( string $url, string $base_url ): string {
		...
		if ( str_starts_with( $url, '/' ) ) {
			...
			// La web usa estas rutas como absolutas aunque no empiecen por '/'.
			if ( ( $first && ( $relative === $first || str_starts_with( $relative, $first . '/' ) ) ) || preg_match( '~^(?:data|img|inicio|noticias|recetas|hortalizas-y-conservas)/~i', $relative ) ) {
				$path = '/' . ltrim( $relative, '/' );
			} else {
