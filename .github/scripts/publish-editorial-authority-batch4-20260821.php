<?php
/** Publish editorial authority batch 4 from repository data files. */
...
function emdo_ab4_category( string $name, string $slug, string $en_name, string $en_slug ): int {
...
function emdo_ab4_image( int $post_id, array $img ): int {
...
function emdo_ab4_product_slug( string $type ): string {
    $candidates = array(
        'hams'=>array('jamones-paletas'), 'oil'=>array('aceites'), 'meat'=>array('carnes','carne'), 'pulses'=>array('legumbres')
    );
    foreach ( $candidates[$type] ?? array() as $slug ) {
        $term = get_term_by( 'slug', $slug, 'product_cat' ); if ( $term instanceof WP_Term ) { return $term->slug; }
    }
    $names = array('hams'=>array('Jamones y paletas'),'oil'=>array('Aceites'),'meat'=>array('Carnes'),'pulses'=>array('Legumbres'));
...
function emdo_ab4_products( string $html ): string {
    $map = array('__HAMS__'=>emdo_ab4_product_slug('hams'),'__OIL__'=>emdo_ab4_product_slug('oil'),'__MEAT__'=>emdo_ab4_product_slug('meat'),'__PULSES__'=>emdo_ab4_product_slug('pulses'));
    return str_replace( array_keys($map), array_values($map), $html );
}

$data_dir = (string) getenv( 'EMDO_BATCH4_DIR' );
if ( '' === $data_dir || ! is_dir( $data_dir ) ) { throw new RuntimeException( 'EMDO_BATCH4_DIR missing' ); }
...
$guide_cat = emdo_ab4_category( 'Guías de compra', 'guias-de-compra', 'Buying guides', 'buying-guides' );
$cats = array(
'hams'=>emdo_ab4_category('Jamones y paletas','jamones-y-paletas','Hams and shoulders','hams-and-shoulders'),
'oil'=>emdo_ab4_category('Aceites','aceites','Olive oil','olive-oil'),
'meat'=>emdo_ab4_category('Carnes','carnes','Beef and meat','beef-and-meat'),
'pulses'=>emdo_ab4_category('Legumbres','legumbres','Pulses','pulses')
);
$seen_images = array();
$report = array('batch'=>4,'release'=>'20260821','posts'=>array());
foreach ( $articles as $a ) {
    ...
    $content=emdo_ab4_products($a['content']); $en_content=emdo_ab4_products($a['en_content']);
    $post_id=emdo_ab4_post_id($a['key'],$a['slug']);
    ...
    update_post_meta($post_id,'_emdo_authority_key',$a['key']); update_post_meta($post_id,'_emdo_authority_batch','4');
    ...
    $image_id=emdo_ab4_image($post_id,$a['image']); $meta=wp_get_attachment_metadata($image_id); $w=(int)($meta['width']??0); $h=(int)($meta['height']??0);
    if($w<1200||$h<700){throw new RuntimeException($a['key'].': image too small '.$w.'x'.$h);} $words_es=emdo_ab4_words($content); $words_en=emdo_ab4_words($en_content);
    if($words_es<900||$words_en<800){throw new RuntimeException($a['key'].': article too short ES='.$words_es.' EN='.$words_en);}
    $report['posts'][]=array('key'=>$a['key'],'id'=>$post_id,'status'=>get_post_status($post_id),'slug'=>(string)get_post_field('post_name',$post_id),'en_slug'=>sanitize_title($a['en_slug']),'words_es'=>$words_es,'words_en'=>$words_en,'image_id'=>$image_id,'image_w'=>$w,'image_h'=>$h,'image_source'=>$a['image']['page'],'topic'=>$a['topic']);
}
