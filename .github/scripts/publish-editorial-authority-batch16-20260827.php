<?php
/** Publish editorial authority batch 16 with the central provisional featured image. */
...
function emdo_ab16_category(string $name,string $slug,string $en_name,string $en_slug):int {
...
function emdo_ab16_words(string $html):int {
...
function emdo_ab16_post_id(string $key,string $slug):int {
...
function emdo_ab16_products(string $html):string {
    return strtr($html,array(
        '__HAMS__'=>emdo_ab16_product_slug('hams'),
        '__OILS__'=>emdo_ab16_product_slug('oils'),
        '__MEAT__'=>emdo_ab16_product_slug('meat'),
    ));
}
...
function emdo_ab16_default_image(int $post_id,bool $is_new):array {
    ...
    // New posts and posts already in the provisional pool use the shared image.
    // If an editor later assigns a definitive image, the MU plugin removes the
    // marker and a publisher re-run deliberately leaves that image untouched.
...
$data_dir=(string)getenv('EMDO_BATCH16_DIR');
if(''===$data_dir||!is_dir($data_dir)){throw new RuntimeException('EMDO_BATCH16_DIR missing');}
$only=(string)getenv('EMDO_BATCH16_ONLY_FILE');
if(''!==$only){
    $only=basename($only);$target=trailingslashit($data_dir).$only;
    if(!is_file($target)){throw new RuntimeException('Batch 16 article file not found: '.$only);}
    $files=array($target);
}else{
...
$guide_cat=emdo_ab16_category('Guías y consejos','guias-y-consejos','Guides and advice','guides-and-advice');
$topic_cats=array(
    'hams'=>emdo_ab16_category('Jamones y paletas','jamones-y-paletas','Hams and shoulders','hams-and-shoulders'),
    'oils'=>emdo_ab16_category('Aceites','aceites','Olive oils','olive-oils'),
    'meat'=>emdo_ab16_category('Carnes','carnes','Meat','meat'),
);
$report=array('batch'=>16,'release'=>'20260827','posts'=>array());
...
foreach($articles as $a){
    ...
    $existing_id=emdo_ab16_post_id($a['key'],$a['slug']);$is_new=$existing_id<=0;
    ...
    update_post_meta($post_id,'_emdo_authority_key',$a['key']);update_post_meta($post_id,'_emdo_authority_batch','16');
    ...
    $img=emdo_ab16_default_image($post_id,$is_new);
