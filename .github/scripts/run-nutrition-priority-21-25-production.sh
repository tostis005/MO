...
Path('/tmp/nutrition-2125.json').write_text(json.dumps(data,ensure_ascii=False),encoding='utf-8')
...
mapfile -t TARGET_SLUGS < <(jq -r '.[].slug' /tmp/nutrition-2125.json)
[ "${#TARGET_SLUGS[@]}" -eq 5 ]
...
trap - ERR; set +e
sshpass -e ssh $SSH "$STAGING_USER@$STAGING_HOST" "export EMDO_NUTRITION_2125_SEED_DIR='$REMOTE'; export PATH='$(dirname "$PHP")':\$PATH; '$PHP' -d memory_limit=768M '$WP' eval-file '$REMOTE/publish-nutrition-priority-21-25-production.php' --path='$PROD' --allow-root" >/tmp/nutrition-2125.raw 2>/tmp/nutrition-2125.err
PUBLISH_RC=$?; set -e; trap rollback_only_new_posts ERR
[ ! -s /tmp/nutrition-2125.err ] || cat /tmp/nutrition-2125.err >&2
[ ! -s /tmp/nutrition-2125.raw ] || cat /tmp/nutrition-2125.raw
[ "$PUBLISH_RC" -eq 0 ]
JSON_LINE="$(grep -E '^\{.*\}$' /tmp/nutrition-2125.raw|tail -n1 || true)"; [ -n "$JSON_LINE" ]
printf '%s\n' "$JSON_LINE" >/tmp/nutrition-2125-result.json
jq -e '.verified==true and .count==5 and (.errors|length)==0 and (.posts|length)==5' /tmp/nutrition-2125-result.json >/dev/null
...
while IFS=$'\t' read -r slug en_slug title en_title permalink en_permalink category_slug; do
  ...
  cat_file="/tmp/category-${category_slug}-${slug}.html"; curl -L -sS --retry 3 -A "$UA" "https://www.elmercadodeorigen.com/category/${category_slug}/?emdo_verify=${GITHUB_RUN_ID}" -o "$cat_file"; grep -Fq "$slug" "$cat_file"
  echo "post_ok es=${permalink} en=${en_permalink} category=${category_slug}"
done < <(jq -r '.posts[]|[.slug,.en_slug,.title,.en_title,.permalink,.en_permalink,.category_slug]|@tsv' /tmp/nutrition-2125-result.json)

curl -L -sS --retry 3 -A "$UA" "https://www.elmercadodeorigen.com/blog/?emdo_verify=${GITHUB_RUN_ID}" -o /tmp/blog.html
while read -r slug; do grep -Fq "$slug" /tmp/blog.html; done < <(jq -r '.posts[].slug' /tmp/nutrition-2125-result.json)
