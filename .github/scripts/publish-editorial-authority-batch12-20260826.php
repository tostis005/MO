<?php
/** Publish editorial authority batch 12 from repository data files. */
...
function emdo_ab12_category(string $name,string $slug,string $en_name,string $en_slug):int {
...
function emdo_ab12_words(string $html):int {
...
function emdo_ab12_post_id(string $key,string $slug):int {
...
function emdo_ab12_image(int $post_id,array $img):int {
    ...
    if($attachment_id<=0){
        ...
        if(is_wp_error($attachment_id)){throw new RuntimeException('Image '.$source_key.': '.$attachment_id->get_error_message());}
        $attachment_id=(int)$attachment_id;
        wp_update_post(array(
            'ID'=>$attachment_id,
            'post_title'=>$img['alt_es'],
            'post_excerpt'=>'Fotografía: '.$img['photographer'].'. Fuente: '.$img['page'].'. Licencia: '.$img['license'].'.'
        ));
...
function emdo_ab12_product_slug():string {
    foreach(array('jamones-paletas') as $slug){$term=get_term_by('slug',$slug,'product_cat');if($term instanceof WP_Term){return $term->slug;}}
    foreach(array('Jamones y paletas') as $name){$term=get_term_by('name',$name,'product_cat');if($term instanceof WP_Term){return $term->slug;}}
    throw new RuntimeException('Ham product category not found');
}

function emdo_ab12_products(string $html):string {
    return str_replace('__HAMS__',emdo_ab12_product_slug(),$html);
}

$data_dir=(string)getenv('EMDO_BATCH12_DIR');
if(''===$data_dir||!is_dir($data_dir)){throw new RuntimeException('EMDO_BATCH12_DIR missing');}
$only=(string)getenv('EMDO_BATCH12_ONLY_FILE');
if(''!==$only){
    $only=basename($only);
    $target=trailingslashit($data_dir).$only;
    if(!is_file($target)){throw new RuntimeException('Batch 12 article file not found: '.$only);}
    $files=array($target);
}else{
...
$guide_cat=emdo_ab12_category('Guías y consejos','guias-y-consejos','Guides and advice','guides-and-advice');
$ham_cat=emdo_ab12_category('Jamones y paletas','jamones-y-paletas','Hams and shoulders','hams-and-shoulders');
$seen_images=array();
$report=array('batch'=>12,'release'=>'20260826','posts'=>array());

foreach($articles as $a){
    foreach(array('key','slug','en_slug','topic','title','en_title','excerpt','en_excerpt','content','en_content','image') as $field){
        if(!isset($a[$field])||''===$a[$field]){throw new RuntimeException(($a['key']??basename($file)).': missing '.$field);}
    }
    if('hams'!==$a['topic']){throw new RuntimeException($a['key'].': unexpected topic '.$a['topic']);}
    ...
    $content=emdo_ab12_products($a['content']);
    $en_content=emdo_ab12_products($a['en_content']);
    $words_es=emdo_ab12_words($content);
    $words_en=emdo_ab12_words($en_content);
    if($words_es<850||$words_en<750){throw new RuntimeException($a['key'].': article too short ES='.$words_es.' EN='.$words_en);}

    $post_id=emdo_ab12_post_id($a['key'],$a['slug']);
    $data=array(
        'post_type'=>'post',
        'post_status'=>'publish',
        ...
        'post_excerpt'=>wp_strip_all_tags($a['excerpt']),
    ...
    $post_id=(int)$result;
    wp_set_post_categories($post_id,array($guide_cat,$ham_cat),false);
    update_post_meta($post_id,'_emdo_authority_key',$a['key']);
    update_post_meta($post_id,'_emdo_authority_batch','12');
    ...
    $image_id=emdo_ab12_image($post_id,$a['image']);
    ...
    $report['posts'][]=array(
        ...
        'words_es'=>$words_es,'words_en'=>$words_en,'image_id'=>$image_id,'image_w'=>$w,'image_h'=>$h,
        'image_source'=>$a['image']['page'],'image_license'=>$a['image']['license'],'topic'=>$a['topic']
    );
}
