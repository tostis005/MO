<?php
/** Publish editorial authority batch 13 from repository data files. */
...
function emdo_ab13_category(string $name,string $slug,string $en_name,string $en_slug):int {
...
function emdo_ab13_words(string $html):int {
...
function emdo_ab13_post_id(string $key,string $slug):int {
...
function emdo_ab13_download_image(string $url,int $post_id,string $title):int {
    $tmp=download_url($url,90);
    if(is_wp_error($tmp)){throw new RuntimeException('Image download failed: '.$tmp->get_error_message());}
    $file=array('name'=>'emdo-batch13-'.$post_id.'-'.time().'.jpg','tmp_name'=>$tmp);
...
function emdo_ab13_image(int $post_id,array $img):int {
    ...
    if($attachment_id<=0){
        $attachment_id=emdo_ab13_download_image((string)$img['direct'],$post_id,(string)$img['alt_es']);
...
function emdo_ab13_product_slug(string $topic):string {
    $map=array(
        'hams'=>array(array('jamones-paletas'),array('Jamones y paletas')),
        'oils'=>array(array('aceites'),array('Aceites')),
        'preserves'=>array(array('conservas'),array('Conservas')),
        'vegetables'=>array(array('hortalizas-verduras','hortalizas','verduras'),array('Hortalizas/Verduras','Hortalizas y verduras','Hortalizas')),
    );
...
function emdo_ab13_products(string $html):string {
    $repl=array(
        '__HAMS__'=>emdo_ab13_product_slug('hams'),
        '__OILS__'=>emdo_ab13_product_slug('oils'),
        '__PRESERVES__'=>emdo_ab13_product_slug('preserves'),
        '__VEGETABLES__'=>emdo_ab13_product_slug('vegetables'),
    );
    return strtr($html,$repl);
}

$data_dir=(string)getenv('EMDO_BATCH13_DIR');
if(''===$data_dir||!is_dir($data_dir)){throw new RuntimeException('EMDO_BATCH13_DIR missing');}
$only=(string)getenv('EMDO_BATCH13_ONLY_FILE');
if(''!==$only){
    $only=basename($only);
    $target=trailingslashit($data_dir).$only;
    if(!is_file($target)){throw new RuntimeException('Batch 13 article file not found: '.$only);}
    $files=array($target);
}else{
...
$guide_cat=emdo_ab13_category('Guías y consejos','guias-y-consejos','Guides and advice','guides-and-advice');
$topic_cats=array(
    'hams'=>emdo_ab13_category('Jamones y paletas','jamones-y-paletas','Hams and shoulders','hams-and-shoulders'),
    'oils'=>emdo_ab13_category('Aceites','aceites','Olive oils','olive-oils'),
    'preserves'=>emdo_ab13_category('Conservas','conservas','Preserves','preserves'),
    'vegetables'=>emdo_ab13_category('Hortalizas y verduras','hortalizas-y-verduras','Vegetables','vegetables'),
);
$seen_images=array();
$report=array('batch'=>13,'release'=>'20260826','posts'=>array());
...
foreach($articles as $a){
    ...
    $content=emdo_ab13_products($a['content']);
    $en_content=emdo_ab13_products($a['en_content']);
    $words_es=emdo_ab13_words($content);
    $words_en=emdo_ab13_words($en_content);
    if($words_es<850||$words_en<750){throw new RuntimeException($a['key'].': article too short ES='.$words_es.' EN='.$words_en);}

    $post_id=emdo_ab13_post_id($a['key'],$a['slug']);
    ...
    $post_id=(int)$result;
    wp_set_post_categories($post_id,array($guide_cat,$topic_cats[$a['topic']]),false);
    update_post_meta($post_id,'_emdo_authority_key',$a['key']);
    update_post_meta($post_id,'_emdo_authority_batch','13');
    ...
    $image_id=emdo_ab13_image($post_id,$a['image']);
