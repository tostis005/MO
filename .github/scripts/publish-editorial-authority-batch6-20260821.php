<?php
/** Publish editorial authority batch 6 from repository data files. */
...
function emdo_ab6_category( string $name, string $slug, string $en_name, string $en_slug ): int {
...
function emdo_ab6_image( int $post_id, array $img ): int {
...
function emdo_ab6_product_slug( string $type ): string {
    $candidates = array(
    ...
    $names = array('hams'=>array('Jamones y paletas'),'oil'=>array('Aceites'),'pulses'=>array('Legumbres'));
...
function emdo_ab6_products( string $html ): string {
    $map = array('__HAMS__'=>emdo_ab6_product_slug('hams'),'__OIL__'=>emdo_ab6_product_slug('oil'),'__PULSES__'=>emdo_ab6_product_slug('pulses'));
    return str_replace( array_keys($map), array_values($map), $html );
}

$data_dir = (string) getenv( 'EMDO_BATCH6_DIR' );
if ( '' === $data_dir || ! is_dir( $data_dir ) ) { throw new RuntimeException( 'EMDO_BATCH6_DIR missing' ); }
...
$guide_cat = emdo_ab6_category( 'Guías de compra', 'guias-de-compra', 'Buying guides', 'buying-guides' );
$cats = array(
'hams'=>emdo_ab6_category('Jamones y paletas','jamones-y-paletas','Hams and shoulders','hams-and-shoulders'),
'oil'=>emdo_ab6_category('Aceites','aceites','Olive oil','olive-oil'),
'pulses'=>emdo_ab6_category('Legumbres','legumbres','Pulses','pulses')
);
$seen_images = array();
$report = array('batch'=>6,'release'=>'20260821','posts'=>array());
foreach ( $articles as $a ) {
    ...
    $content=emdo_ab6_products($a['content']); $en_content=emdo_ab6_products($a['en_content']);
    $post_id=emdo_ab6_post_id($a['key'],$a['slug']);
    ...
    update_post_meta($post_id,'_emdo_authority_key',$a['key']); update_post_meta($post_id,'_emdo_authority_batch','6');
    ...
    $image_id=emdo_ab6_image($post_id,$a['image']); $meta=wp_get_attachment_metadata($image_id); $w=(int)($meta['width']??0); $h=(int)($meta['height']??0);
    if($w<1200||$h<700){throw new RuntimeException($a['key'].': image too small '.$w.'x'.$h);} $words_es=emdo_ab6_words($content); $words_en=emdo_ab6_words($en_content);
