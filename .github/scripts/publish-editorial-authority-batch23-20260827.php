<?php
/** Publish editorial authority batch 23 with the central provisional featured image. */
...
function emdo_ab23_category(string $name,string $slug,string $en_name,string $en_slug):int{
...
function emdo_ab23_words(string $html):int{$plain=trim(preg_replace('/\s+/u',' ',wp_strip_all_tags(strip_shortcodes($html))));if(''===$plain){return 0;}preg_match_all("/[\\p{L}\\p{M}]+(?:[’'’-][\\p{L}\\p{M}]+)*/u",$plain,$m);return count($m[0]);}
function emdo_ab23_post_id(string $key,string $slug):int{$ids=get_posts(array('post_type'=>'post','post_status'=>array('publish','draft','pending','future','private'),'posts_per_page'=>1,'fields'=>'ids','meta_key'=>'_emdo_authority_key','meta_value'=>$key));if(!empty($ids)){return (int)$ids[0];}$post=get_page_by_path($slug,OBJECT,'post');return $post instanceof WP_Post?(int)$post->ID:0;}
function emdo_ab23_product_slug():string{
 foreach(array('hortalizas-verduras','hortalizas','verduras') as $slug){$term=get_term_by('slug',$slug,'product_cat');if($term instanceof WP_Term){return $term->slug;}}
 foreach(array('Hortalizas/Verduras','Hortalizas y verduras','Hortalizas','Vegetables') as $name){$term=get_term_by('name',$name,'product_cat');if($term instanceof WP_Term){return $term->slug;}}
 throw new RuntimeException('Vegetable product category not found.');
}
function emdo_ab23_products(string $html):string{return str_replace('__VEGETABLES__',emdo_ab23_product_slug(),$html);}
function emdo_ab23_default_image(int $post_id,bool $is_new):array{
...
$data_dir=(string)getenv('EMDO_BATCH23_DIR');if(''===$data_dir||!is_dir($data_dir)){throw new RuntimeException('EMDO_BATCH23_DIR missing');}
$only=(string)getenv('EMDO_BATCH23_ONLY_FILE');if(''!==$only){$only=basename($only);$target=trailingslashit($data_dir).$only;if(!is_file($target)){throw new RuntimeException('Batch 23 article file not found: '.$only);}$files=array($target);}else{$files=glob(trailingslashit($data_dir).'*.php');sort($files,SORT_NATURAL);if(count($files)!==5){throw new RuntimeException('Expected 5 article files, found '.count($files));}}
...
$guide_cat=emdo_ab23_category('Guías y consejos','guias-y-consejos','Guides and advice','guides-and-advice');
$vegetable_cat=emdo_ab23_category('Hortalizas y verduras','hortalizas-y-verduras','Vegetables','vegetables');
$report=array('batch'=>23,'release'=>'20260827','posts'=>array());
foreach($articles as $a){
 ...
 $content=emdo_ab23_products($a['content']);$en_content=emdo_ab23_products($a['en_content']);
 $words_es=emdo_ab23_words($content);$words_en=emdo_ab23_words($en_content);if($words_es<850||$words_en<750){throw new RuntimeException($a['key'].': article too short ES='.$words_es.' EN='.$words_en);}
 $existing_id=emdo_ab23_post_id($a['key'],$a['slug']);$is_new=$existing_id<=0;$data=array('post_type'=>'post','post_status'=>'publish','post_author'=>$author_id,'post_title'=>wp_strip_all_tags($a['title']),'post_name'=>sanitize_title($a['slug']),'post_excerpt'=>wp_strip_all_tags($a['excerpt']),'post_content'=>$content);if(!$is_new){$data['ID']=$existing_id;$result=wp_update_post($data,true);}else{$result=wp_insert_post($data,true);}if(is_wp_error($result)){throw new RuntimeException($a['key'].': '.$result->get_error_message());}
 $post_id=(int)$result;wp_set_post_categories($post_id,array($guide_cat,$vegetable_cat),false);update_post_meta($post_id,'_emdo_authority_key',$a['key']);update_post_meta($post_id,'_emdo_authority_batch','23');update_post_meta($post_id,'_en_US_post_title',wp_strip_all_tags($a['en_title']));update_post_meta($post_id,'_en_US_post_name',sanitize_title($a['en_slug']));update_post_meta($post_id,'_en_US_post_excerpt',wp_strip_all_tags($a['en_excerpt']));update_post_meta($post_id,'_en_US_post_content',$en_content);update_post_meta($post_id,'_en_US_ready','1');update_post_meta($post_id,'_en_US_published','1');update_post_meta($post_id,'_emdo_editorial_updated',gmdate('c'));
 $img=emdo_ab23_default_image($post_id,$is_new);if('publish'!==get_post_status($post_id)){throw new RuntimeException($a['key'].': post is not published');}if('1'!==get_post_meta($post_id,'_en_US_published',true)){throw new RuntimeException($a['key'].': English version not marked published');}
