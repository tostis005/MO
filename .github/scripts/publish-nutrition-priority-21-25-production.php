<?php
/** Publish bilingual nutrition priority articles 21-25. */
if ( ! defined( 'ABSPATH' ) ) { exit( 1 ); }
$seed_dir = getenv( 'EMDO_NUTRITION_2125_SEED_DIR' );
if ( ! is_string( $seed_dir ) || '' === trim( $seed_dir ) || ! is_dir( $seed_dir ) ) { WP_CLI::error( 'Invalid seed directory.' ); }
$seed_dir = rtrim( $seed_dir, '/\\' );

function emdo_2125_articles( string $dir ): array {
    $encoded='';
    for($i=1;$i<=4;$i++){
        $file=$dir.'/content-seeds/nutrition-priority-21-25-010271-'.$i.'.b64';
    ...
    foreach($data as $a){ foreach($required as $k){ if(!isset($a[$k]) || (is_string($a[$k]) && ''===trim($a[$k]))){ WP_CLI::error('Missing '.$k.' in '.$a['slug']); } } }
    return $data;
}
function emdo_2125_category_id(array $a): int {
...
function emdo_2125_product_slugs(array $a): array {
    $out=array();
...
function emdo_2125_render(array $a,array $product_slugs,bool $en=false): string {
...
function emdo_2125_image_id(): int {
...
function emdo_2125_save_en(int $id,array $a,string $content): void {
...
function emdo_2125_remove_exact_stale(array $a): void {
    $slug=(string)$a['slug'];
    ...
    if(!$same_title || !$same_excerpt){
        WP_CLI::error('Safety stop: existing slug is not an exact batch 21-25 residue: '.$slug.' id='.(int)$existing->ID);
    }
...
$articles=emdo_2125_articles($seed_dir); $image_id=emdo_2125_image_id();
if($image_id<=0){ WP_CLI::error('Generic provisional image not found.'); }
$rows=array(); $errors=array();
foreach($articles as $a){
    $slug=(string)$a['slug'];
    emdo_2125_remove_exact_stale($a);
    ...
    $cat_id=emdo_2125_category_id($a); if($cat_id<=0){ WP_CLI::error('Blog category not found: '.$a['category_slug']); }
    $product_slugs=emdo_2125_product_slugs($a);
    $es=emdo_2125_render($a,$product_slugs,false); $en=emdo_2125_render($a,$product_slugs,true);
    ...
    $id=(int)$r; set_post_thumbnail($id,$image_id); emdo_2125_save_en($id,$a,$en);
    $p=get_post($id);
    ...
    $rows[]=array('id'=>$id,'slug'=>$slug,'en_slug'=>(string)$a['en_slug'],'title'=>(string)$a['title'],'en_title'=>(string)$a['en_title'],'category_slug'=>(string)$a['category_slug'],'permalink'=>(string)get_permalink($id),'en_permalink'=>(string)home_url('/en/'.trim((string)$a['en_slug'],'/').'/'),'thumbnail_id'=>(int)get_post_thumbnail_id($id),'product_cats'=>$product_slugs);
}
...
if(!$out['verified']){ WP_CLI::error('Batch 21-25 verification failed.'); }
