<?php
/** Publish editorial authority batch 14 from repository data files. */
...
function emdo_ab14_category(string $name,string $slug,string $en_name,string $en_slug):int {
...
function emdo_ab14_words(string $html):int {
...
function emdo_ab14_post_id(string $key,string $slug):int {
...
function emdo_ab14_download_image(string $url,int $post_id,string $title):int {
    $tmp=download_url($url,90);
    if(is_wp_error($tmp)){throw new RuntimeException('Image download failed: '.$tmp->get_error_message());}
    $file=array('name'=>'emdo-batch14-'.$post_id.'-'.time().'.jpg','tmp_name'=>$tmp);
...
function emdo_ab14_image(int $post_id,array $img):int {
    ...
    if($attachment_id<=0){
        $attachment_id=emdo_ab14_download_image((string)$img['direct'],$post_id,(string)$img['alt_es']);
...
function emdo_ab14_product_slug(string $topic):string {
    $map=array(
...
function emdo_ab14_products(string $html):string {
    $repl=array(
        '__CURED_MEATS__'=>emdo_ab14_product_slug('cured_meats'),
        '__VEGETABLES__'=>emdo_ab14_product_slug('vegetables'),
    );
    return strtr($html,$repl);
}

$data_dir=(string)getenv('EMDO_BATCH14_DIR');
if(''===$data_dir||!is_dir($data_dir)){throw new RuntimeException('EMDO_BATCH14_DIR missing');}
$only=(string)getenv('EMDO_BATCH14_ONLY_FILE');
if(''!==$only){
    $only=basename($only);
    $target=trailingslashit($data_dir).$only;
    if(!is_file($target)){throw new RuntimeException('Batch 14 article file not found: '.$only);}
    $files=array($target);
}else{
    $files=glob(trailingslashit($data_dir).'*.php');
    sort($files,SORT_NATURAL);
    if(count($files)!==4){throw new RuntimeException('Expected 4 article files, found '.count($files));}
}
...
$guide_cat=emdo_ab14_category('Guías y consejos','guias-y-consejos','Guides and advice','guides-and-advice');
$topic_cats=array(
    'cured_meats'=>emdo_ab14_category('Embutidos y curados','embutidos-y-curados','Cured meats','cured-meats'),
    'vegetables'=>emdo_ab14_category('Hortalizas y verduras','hortalizas-y-verduras','Vegetables','vegetables'),
);
$seen_images=array();
$report=array('batch'=>14,'release'=>'20260826','posts'=>array());
...
foreach($articles as $a){
    ...
    $content=emdo_ab14_products($a['content']);
    $en_content=emdo_ab14_products($a['en_content']);
    $words_es=emdo_ab14_words($content);
    $words_en=emdo_ab14_words($en_content);
    if($words_es<850||$words_en<750){throw new RuntimeException($a['key'].': article too short ES='.$words_es.' EN='.$words_en);}

    $post_id=emdo_ab14_post_id($a['key'],$a['slug']);
    ...
    update_post_meta($post_id,'_emdo_authority_batch','14');
