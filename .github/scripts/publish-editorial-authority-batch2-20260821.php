<?php
/** Publish editorial authority batch 2 from repository data files. */
...
function emdo_ab2_category( string $name, string $slug, string $en_name, string $en_slug ): int {
...
function emdo_ab2_words( string $html ): int {
...
function emdo_ab2_post_id( string $key, string $slug ): int {
    $ids = get_posts( array(
...
function emdo_ab2_image( int $post_id, array $img ): int {
    $ids = get_posts( array(
...
function emdo_ab2_product_slug( string $topic ): string {
    $candidates = array(
        'oil' => array( 'aceites' ),
        'meat' => array( 'carnes', 'carne' ),
        'vegetables' => array( 'hortalizas-verduras', 'hortalizas', 'verduras' ),
        'pulses' => array( 'legumbres' ),
    );
    foreach ( $candidates[ $topic ] ?? array() as $slug ) {
    ...
    foreach ( $names[ $topic ] ?? array() as $name ) {
        $term = get_term_by( 'name', $name, 'product_cat' );
        if ( $term instanceof WP_Term ) { return $term->slug; }
    }
    throw new RuntimeException( 'Product category not found for topic ' . $topic );
}

function emdo_ab2_normalize_products( string $html, string $topic ): string {
    $slug = emdo_ab2_product_slug( $topic );
    return preg_replace( '/\[products\s+category="[^"]+"/', '[products category="' . esc_attr( $slug ) . '"', $html );
}

$data_dir = (string) getenv( 'EMDO_BATCH2_DIR' );
if ( '' === $data_dir || ! is_dir( $data_dir ) ) { throw new RuntimeException( 'EMDO_BATCH2_DIR missing' ); }
$files = glob( trailingslashit( $data_dir ) . '*.php' );
...
$guide_cat = emdo_ab2_category( 'Guías de compra', 'guias-de-compra', 'Buying guides', 'buying-guides' );
$cats = array(
    'oil' => emdo_ab2_category( 'Aceites', 'aceites', 'Olive oil', 'olive-oil' ),
    'meat' => emdo_ab2_category( 'Carnes', 'carnes', 'Beef and meat', 'beef-and-meat' ),
    'vegetables' => emdo_ab2_category( 'Hortalizas y verduras', 'hortalizas-y-verduras', 'Vegetables', 'vegetables' ),
    'pulses' => emdo_ab2_category( 'Legumbres', 'legumbres', 'Pulses', 'pulses' ),
);

$seen_images = array();
$report = array( 'batch' => 2, 'release' => '20260821', 'posts' => array() );
foreach ( $articles as $a ) {
    ...
    $content = emdo_ab2_normalize_products( $a['content'], $a['topic'] );
    $en_content = emdo_ab2_normalize_products( $a['en_content'], $a['topic'] );
    $post_id = emdo_ab2_post_id( $a['key'], $a['slug'] );
    $data = array(
    ...
    update_post_meta( $post_id, '_emdo_authority_batch', '2' );
    ...
    $image_id = emdo_ab2_image( $post_id, $a['image'] );
    $meta = wp_get_attachment_metadata( $image_id );
    $w = (int) ( $meta['width'] ?? 0 );
    $h = (int) ( $meta['height'] ?? 0 );
    if ( $w < 1200 || $h < 700 ) { throw new RuntimeException( $a['key'] . ': image too small ' . $w . 'x' . $h ); }
    $words_es = emdo_ab2_words( $content );
    $words_en = emdo_ab2_words( $en_content );
    ...
    $report['posts'][] = array(
        ...
        'image_w' => $w, 'image_h' => $h, 'image_source' => $a['image']['page'],
        'product_cat' => emdo_ab2_product_slug( $a['topic'] ),
    );
}
clean_post_cache( 0 );
