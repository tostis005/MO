<?php
/**
 * Publish and verify EMDO blog expansion batch 03 (10 bilingual articles).
 * Spanish WordPress posts + Falang en_US metadata, related WooCommerce products,
 * and the established generic featured image for every post.
 */
if ( ! defined( 'ABSPATH' ) ) { exit( 1 ); }

const EMDO_BLOG_EXPANSION_03_MARKER = '2026-09-07.blog-expansion-03.v1';
const EMDO_BLOG_DEFAULT_FEATURED_IMAGE_ID = 13442;

function emdo_blog03_load_articles(): array {
    $encoded = '';
    for ( $part = 1; $part <= 4; $part++ ) {
        $path = __DIR__ . '/blog-expansion-03-' . $part . '.b64';
    ...
    sort( $positions );
    if ( $positions !== range( 21, 30 ) ) { throw new RuntimeException( 'Editorial positions must be 21 through 30.' ); }
    return $articles;
}

function emdo_blog03_category( array $article ): WP_Term {
...
function emdo_blog03_product_category( array $article ): WP_Term {
...
function emdo_blog03_author_for_category( int $category_id ): int {
...
function emdo_blog03_render_content( array $article, WP_Term $product_cat, bool $english ): string {
...
function emdo_blog03_trim( string $text, int $max ): string {
...
foreach ( $articles as $article ) {
    $category = emdo_blog03_category( $article );
    $product_cat = emdo_blog03_product_category( $article );
    $author = emdo_blog03_author_for_category( (int) $category->term_id );
    ...
    $id = (int) $result;
    update_post_meta( $id, '_emdo_blog_expansion_03', EMDO_BLOG_EXPANSION_03_MARKER );
...
foreach ( $articles as $article ) {
    ...
    $spanish = emdo_blog03_render_content( $article, $product_cat, false );
    $english = emdo_blog03_render_content( $article, $product_cat, true );
    $es_words = emdo_blog03_word_count( $spanish ); $en_words = emdo_blog03_word_count( $english );
    ...
    update_post_meta( $id, '_emdo_seo_title', emdo_blog03_trim( (string) $article['seo_title'], 62 ) );
    update_post_meta( $id, '_emdo_seo_description', emdo_blog03_trim( (string) $article['seo_description'], 158 ) );
    update_post_meta( $id, '_en_US_seo_title', emdo_blog03_trim( (string) $article['en_seo_title'], 62 ) );
    update_post_meta( $id, '_en_US_seo_description', emdo_blog03_trim( (string) $article['en_seo_description'], 158 ) );
    update_post_meta( $id, 'rank_math_title', emdo_blog03_trim( (string) $article['seo_title'], 62 ) );
    update_post_meta( $id, 'rank_math_description', emdo_blog03_trim( (string) $article['seo_description'], 158 ) );
...
$out = array(
    'marker'=>EMDO_BLOG_EXPANSION_03_MARKER,'verified'=>true,'count'=>count($rows),
    'default_featured_image_id'=>$default_image_id,'posts'=>array_values($rows),'errors'=>array()
);
echo "EMDO_BLOG_EXPANSION_03_BEGIN\n";
echo wp_json_encode( $out, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) . "\n";
echo "EMDO_BLOG_EXPANSION_03_END\n";
