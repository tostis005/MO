<?php
/** Publish editorial authority batch 3 from repository data files. */
...
function emdo_ab3_category( string $name, string $slug, string $en_name, string $en_slug ): int {
...
function emdo_ab3_words( string $html ): int {
...
function emdo_ab3_post_id( string $key, string $slug ): int {
    $ids = get_posts( array(
...
function emdo_ab3_image( int $post_id, array $img ): int {
    $ids = get_posts( array(
        'post_type' => 'attachment', 'post_status' => 'inherit', 'posts_per_page' => 1, 'fields' => 'ids',
        'meta_key' => '_emdo_pexels_photo_id', 'meta_value' => (string) $img['id'],
    ) );
...
function emdo_ab3_product_slug( string $type ): string {
    $candidates = array(
        'hams' => array( 'jamones-paletas' ),
        'cured' => array( 'embutidos-y-curados', 'embutidos' ),
        'oil' => array( 'aceites' ),
...
function emdo_ab3_products( string $html ): string {
    $map = array(
        '__HAMS__' => emdo_ab3_product_slug( 'hams' ),
        '__CURED__' => emdo_ab3_product_slug( 'cured' ),
        '__OIL__' => emdo_ab3_product_slug( 'oil' ),
        '__MEAT__' => emdo_ab3_product_slug( 'meat' ),
        '__VEGETABLES__' => emdo_ab3_product_slug( 'vegetables' ),
    );
    return str_replace( array_keys( $map ), array_values( $map ), $html );
}

$data_dir = (string) getenv( 'EMDO_BATCH3_DIR' );
if ( '' === $data_dir || ! is_dir( $data_dir ) ) { throw new RuntimeException( 'EMDO_BATCH3_DIR missing' ); }
...
$guide_cat = emdo_ab3_category( 'Guías de compra', 'guias-de-compra', 'Buying guides', 'buying-guides' );
$cats = array(
    'hams' => emdo_ab3_category( 'Jamones y paletas', 'jamones-y-paletas', 'Hams and shoulders', 'hams-and-shoulders' ),
    'cured' => emdo_ab3_category( 'Embutidos y curados', 'embutidos-y-curados', 'Cured meats', 'cured-meats' ),
    'oil' => emdo_ab3_category( 'Aceites', 'aceites', 'Olive oil', 'olive-oil' ),
    'meat' => emdo_ab3_category( 'Carnes', 'carnes', 'Beef and meat', 'beef-and-meat' ),
    'vegetables' => emdo_ab3_category( 'Hortalizas y verduras', 'hortalizas-y-verduras', 'Vegetables', 'vegetables' ),
);

$seen_images = array();
$report = array( 'batch' => 3, 'release' => '20260821', 'posts' => array() );
foreach ( $articles as $a ) {
    ...
    $image_key = (string) $a['image']['id'];
    if ( isset( $seen_images[ $image_key ] ) ) { throw new RuntimeException( 'Repeated image id ' . $image_key ); }
    $seen_images[ $image_key ] = true;
    $content = emdo_ab3_products( $a['content'] );
    $en_content = emdo_ab3_products( $a['en_content'] );
    $post_id = emdo_ab3_post_id( $a['key'], $a['slug'] );
    $data = array(
    ...
    update_post_meta( $post_id, '_emdo_authority_batch', '3' );
    ...
    $report['posts'][] = array(
        ...
        'words_es'=>$words_es, 'words_en'=>$words_en, 'image_id'=>$image_id,
        'image_w'=>$w, 'image_h'=>$h, 'image_source'=>$a['image']['page'], 'topic'=>$a['topic'],
    );
}
