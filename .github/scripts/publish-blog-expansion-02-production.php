<?php
/**
 * Publish and verify EMDO blog expansion batch 02 (10 bilingual articles).
 * Spanish WordPress posts + Falang en_US metadata, with related WooCommerce products.
 */
if ( ! defined( 'ABSPATH' ) ) { exit( 1 ); }

const EMDO_BLOG_EXPANSION_02_MARKER = '2026-09-07.blog-expansion-02.v1';
const EMDO_BLOG_DEFAULT_FEATURED_IMAGE_ID = 13442;

function emdo_blog02_load_articles(): array {
    $encoded = '';
    for ( $part = 1; $part <= 4; $part++ ) {
        $path = __DIR__ . '/blog-expansion-02-' . $part . '.b64';
    ...
    sort( $positions );
    if ( $positions !== range( 11, 20 ) ) { throw new RuntimeException( 'Editorial positions must be 11 through 20.' ); }
    return $articles;
}

function emdo_blog02_category( array $article ): WP_Term {
...
function emdo_blog02_product_category( array $article ): WP_Term {
...
function emdo_blog02_author_for_category( int $category_id ): int {
...
function emdo_blog02_render_content( array $article, WP_Term $product_cat, bool $english ): string {
...
function emdo_blog02_trim( string $text, int $max ): string {
...
foreach ( $articles as $article ) {
    $category = emdo_blog02_category( $article );
    $product_cat = emdo_blog02_product_category( $article );
    $author = emdo_blog02_author_for_category( (int) $category->term_id );
    ...
    $id = (int) $result;
    update_post_meta( $id, '_emdo_blog_expansion_02', EMDO_BLOG_EXPANSION_02_MARKER );
...
foreach ( $articles as $article ) {
    ...
    $spanish = emdo_blog02_render_content( $article, $product_cat, false );
    $english = emdo_blog02_render_content( $article, $product_cat, true );

    $es_words = emdo_blog02_word_count( $spanish ); $en_words = emdo_blog02_word_count( $english );
    ...
    update_post_meta( $id, '_emdo_seo_title', emdo_blog02_trim( (string) $article['seo_title'], 62 ) );
    update_post_meta( $id, '_emdo_seo_description', emdo_blog02_trim( (string) $article['seo_description'], 158 ) );
    update_post_meta( $id, '_en_US_seo_title', emdo_blog02_trim( (string) $article['en_seo_title'], 62 ) );
    update_post_meta( $id, '_en_US_seo_description', emdo_blog02_trim( (string) $article['en_seo_description'], 158 ) );
    update_post_meta( $id, 'rank_math_title', emdo_blog02_trim( (string) $article['seo_title'], 62 ) );
    update_post_meta( $id, 'rank_math_description', emdo_blog02_trim( (string) $article['seo_description'], 158 ) );
    ...
    if ( $default_image_id !== (int) get_post_thumbnail_id( $id ) ) { throw new RuntimeException( 'Unexpected featured image assigned: ' . $slug ); }
...
$out = array(
    'marker'=>EMDO_BLOG_EXPANSION_02_MARKER,'verified'=>true,'count'=>count($rows),'default_featured_image_id'=>$default_image_id,'posts'=>array_values($rows),'errors'=>array()
);
echo "EMDO_BLOG_EXPANSION_02_BEGIN\n";
echo wp_json_encode( $out, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ) . "\n";
echo "EMDO_BLOG_EXPANSION_02_END\n";
