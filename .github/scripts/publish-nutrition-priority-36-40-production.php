<?php
/** Publish bilingual nutrition priority articles 36-40 (Iberian ham fat + EVOO filtration). */
if ( ! defined( 'ABSPATH' ) ) { exit( 1 ); }
$seed_dir = getenv( 'EMDO_NUTRITION_3640_SEED_DIR' );
...
function emdo_3640_articles( string $dir ): array {
    ...
    foreach($data as $a){
        ...
        if(!in_array((string)$a['category_slug'],array('jamones-y-paletas','aceites'),true)){ WP_CLI::error('Unexpected category: '.$a['category_slug']); }
    }
    return $data;
}
function emdo_3640_category_id(array $a): int {
...
function emdo_3640_product_slugs(array $a): array {
    $out=array();
...
function emdo_3640_render(array $a,array $product_slugs,bool $en=false): string {
...
function emdo_3640_image_id(): int {
...
function emdo_3640_save_en(int $id,array $a,string $content): void {
...
$articles=emdo_3640_articles($seed_dir); $image_id=emdo_3640_image_id();
...
foreach($articles as $a){
    ...
    $cat_id=emdo_3640_category_id($a); if($cat_id<=0){ WP_CLI::error('Blog category not found: '.$a['category_slug']); }
    $product_slugs=emdo_3640_product_slugs($a);
    $es=emdo_3640_render($a,$product_slugs,false); $en=emdo_3640_render($a,$product_slugs,true);
    ...
    $id=(int)$r; set_post_thumbnail($id,$image_id); emdo_3640_save_en($id,$a,$en);
    $p=get_post($id);
...
if(!$out['verified']){ WP_CLI::error('Batch 36-40 verification failed.'); }
