<?php
/**
 * Publish and verify EMDO master-list gap batch 05 (10 bilingual articles).
...
const EMDO_MASTER_GAP_05_MARKER = '2026-09-08.master-gap-05';
const EMDO_BLOG_DEFAULT_FEATURED_IMAGE_ID = 13442;

function emdo_gap05_load_articles(): array {
    $encoded = '';
    ...
    if ( hash('sha256', $compressed) !== 'dbc5bf02234f06037bee506aab77c0e272de91a519dbd8b1c006c41ed2969b7f' ) {
        throw new RuntimeException( 'Compressed editorial payload hash mismatch.' );
    }
    $json = gzdecode( $compressed );
    if ( false === $json ) { throw new RuntimeException( 'Editorial payload cannot be decompressed.' ); }
    if ( hash('sha256', $json) !== 'a401a869f6063541f6256b96b2c88733eda7320c9c447f8340d465324e48cc42' ) {
        throw new RuntimeException( 'Decoded editorial payload hash mismatch.' );
    }
    ...
    $expected = array(33,35,36,37,40,42,43,44,45,46);
...
function emdo_gap05_category( array $article ): WP_Term {
...
function emdo_gap05_product_category( array $article ): WP_Term {
...
function emdo_gap05_author_for_category( int $category_id ): int {
...
function emdo_gap05_render_content( array $article, WP_Term $product_cat, bool $english ): string {
...
foreach ( $articles as $article ) {
    $category = emdo_gap05_category( $article );
    $product_cat = emdo_gap05_product_category( $article );
    $author = emdo_gap05_author_for_category( (int) $category->term_id );
    ...
    $id = (int)$result;
    update_post_meta( $id, '_emdo_master_gap_05', EMDO_MASTER_GAP_05_MARKER );
...
foreach ( $articles as $article ) {
    ...
    $spanish = emdo_gap05_render_content( $article, $product_cat, false );
    $english = emdo_gap05_render_content( $article, $product_cat, true );
    $es_words = emdo_gap05_word_count( $spanish ); $en_words = emdo_gap05_word_count( $english );
    ...
    update_post_meta( $id, '_emdo_seo_title', emdo_gap05_trim((string)$article['seo_title'],62) );
    update_post_meta( $id, '_emdo_seo_description', emdo_gap05_trim((string)$article['seo_description'],158) );
    update_post_meta( $id, '_en_US_seo_title', emdo_gap05_trim((string)$article['en_seo_title'],62) );
    update_post_meta( $id, '_en_US_seo_description', emdo_gap05_trim((string)$article['en_seo_description'],158) );
    update_post_meta( $id, 'rank_math_title', emdo_gap05_trim((string)$article['seo_title'],62) );
    update_post_meta( $id, 'rank_math_description', emdo_gap05_trim((string)$article['seo_description'],158) );
...
$out=array(
    'marker'=>EMDO_MASTER_GAP_05_MARKER,'verified'=>true,'count'=>count($rows),
    'default_featured_image_id'=>$default_image_id,'posts'=>array_values($rows),'errors'=>array()
);
echo "EMDO_MASTER_GAP_05_BEGIN\n";
echo wp_json_encode($out,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)."\n";
echo "EMDO_MASTER_GAP_05_END\n";
