<?php
/** Core-only publisher for editorial authority batch 24. Run with --skip-plugins --skip-themes. */
if (!defined('ABSPATH')) { exit; }
function emdo_ab24c_words(string $html): int {
    $plain=trim(preg_replace('/\s+/u',' ',wp_strip_all_tags($html)));
    if ($plain==='') return 0;
    preg_match_all("/[\\p{L}\\p{M}]+(?:[’'’-][\\p{L}\\p{M}]+)*/u",$plain,$m);
    return count($m[0]);
}
function emdo_ab24c_category(string $name,string $slug,string $en_name,string $en_slug): int {
...
function emdo_ab24c_existing(string $key,string $slug): int {
    global $wpdb;
    ...
    if ($id>0) return $id;
    $id=(int)$wpdb->get_var($wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE post_type='post' AND post_name=%s ORDER BY ID DESC LIMIT 1",sanitize_title($slug)));
    return $id;
}
$data_dir=(string)getenv('EMDO_BATCH24_CORE_DIR');
if ($data_dir==='' || !is_dir($data_dir)) throw new RuntimeException('EMDO_BATCH24_CORE_DIR missing');
...
$guide=emdo_ab24c_category('Guías y consejos','guias-y-consejos','Guides and advice','guides-and-advice');
$vegetables=emdo_ab24c_category('Hortalizas y verduras','hortalizas-y-verduras','Vegetables','vegetables');
$cured=emdo_ab24c_category('Embutidos','embutidos','Cured meats','cured-meats');
$cats=array('vegetables'=>$vegetables,'cured_meats'=>$cured);
...
foreach ($files as $file) {
    ...
    $content=strtr($a['content'],array('__VEGETABLES__'=>'hortalizas-verduras','__CURED_MEATS__'=>'embutidos'));
    $en_content=strtr($a['en_content'],array('__VEGETABLES__'=>'hortalizas-verduras','__CURED_MEATS__'=>'embutidos'));
    $wes=emdo_ab24c_words($content); $wen=emdo_ab24c_words($en_content);
    if($wes<850 || $wen<750) throw new RuntimeException($a['key'].' too short ES='.$wes.' EN='.$wen);
    $id=emdo_ab24c_existing($a['key'],$a['slug']);
    $post=array(
    ...
    if($id>0){$post['ID']=$id;$res=wp_update_post($post,true);}else{$res=wp_insert_post($post,true);}
    ...
    clean_post_cache($id);
    $rows[]=array('key'=>$a['key'],'id'=>$id,'slug'=>get_post_field('post_name',$id),'en_slug'=>sanitize_title($a['en_slug']),'status'=>get_post_status($id),'thumbnail_id'=>(int)get_post_thumbnail_id($id),'provisional'=>(string)get_post_meta($id,'_emdo_uses_default_featured',true),'words_es'=>$wes,'words_en'=>$wen);
}
if(count($rows)!==5) throw new RuntimeException('Publish count mismatch');
echo wp_json_encode(array('batch'=>24,'default'=>$default,'posts'=>$rows),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT).PHP_EOL;
