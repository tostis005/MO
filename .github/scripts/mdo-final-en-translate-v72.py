import json,re,html,time,sys,unicodedata
...
'lomo de cebo de campo':'free-range grain-fed cured loin','lomo de bellota':'acorn-fed cured loin','lomo bellota':'acorn-fed cured loin','lomo embuchado':'cured pork loin',
'lomito de bellota':'acorn-fed presa loin','cinta de lomo':'pork loin','cabecero de lomo':'cured loin head',
'chorizo cular de bellota':'acorn-fed cular chorizo','chorizo cular':'cular chorizo','chorizo de bellota':'acorn-fed chorizo',
'salchichón cular de bellota':'acorn-fed cular salchichón','salchichón cular':'cular salchichón','salchichón de bellota':'acorn-fed salchichón',
...
'raza ibérica':'Iberian breed','pura raza ibérica':'pure Iberian breed','cerdo ibérico':'Iberian pig','cerdos ibéricos':'Iberian pigs',
'cebo de campo':'free-range grain-fed','bellotas':'acorns','bellota':'acorn','dehesa':'dehesa pasture','montanera':'montanera season',
...
'sobres':'packs','sobre':'pack','codillo':'ham hock','punta':'end piece','paleta':'shoulder ham','jamón':'ham','lomo':'cured loin','lomito':'presa loin',
'deshuesado':'boneless','deshuesada':'boneless','loncheado':'sliced','loncheada':'sliced','loncheados':'sliced','loncheadas':'sliced',
'en caja de madera':'in a wooden box','en tubo':'in a presentation tube','lote':'selection','surtido':'assortment','degustación':'tasting','cata':'tasting',
...
MANUAL_TITLES={
'La caja del estudiante':'The Student Box','Cátedra ibérica':'Iberian Masterclass','Universidad ibérica':'Iberian University',
'La cátedra del sabor':'The Flavour Masterclass','El catedrático selección':'El Catedrático Selection','El catedrático gourmet':'El Catedrático Gourmet',
'Catedrático degustación':'El Catedrático Tasting Selection','Tabla jamonera modelo bellota 3':'Bellota Model 3 Ham Stand'
}
...
def polish(s):
    fixes=[
      (r'\bTake 6 and pay 5\b','Buy 6, pay for 5'),(r'\bBuy 6 and pay 5\b','Buy 6, pay for 5'),
      (r'\bsachets\b','packs'),(r'\benvelopes\b','packs'),(r'\bpalette(s)?\b',lambda m:'shoulder hams' if m.group(1) else 'shoulder ham'),
      (r'\bpallets\b','shoulder hams'),(r'\bpallet\b','shoulder ham'),(r'\bIberian race\b','Iberian breed'),(r'\bIberic race\b','Iberian breed'),
      (r'\bhealing process\b','curing process'),(r'\bhealing time\b','curing time'),(r'\bhealing\b','curing'),
      (r'\bcattle ranches\b','farms'),(r'\bbred in freedom\b','raised free-range'),(r'\braised in freedom\b','raised free-range'),
      (r'\bpreferential consumption\b','best before'),(r'\bpreference consumption\b','best before'),
      (r'\bgrs\.?\b','g'),(r'\bgrams\b','g'),(r'\bKj\b','kJ'),(r'\bKcal\b','kcal'),
      (r'\bconservatives\b','preservatives'),(r'\bconservative\b','preservative'),(r'\bcarbon hydrates\b','carbohydrates'),
      (r'\bsit chair\b','set the standard'),(r'\bsits chair\b','sets the standard'),(r'\bset chair\b','set the standard'),
      (r'\bsausage table\b','charcuterie board'),(r'\bempty-packed\b','vacuum-packed'),(r'\bpacked into the vacuum\b','vacuum-packed'),(r'\bpacked to the vacuum\b','vacuum-packed'),
      (r'\bat a single price\b','at a special price')
    ]
...
def gtranslate(s):
    ...
    for attempt in range(6):
        try:
            ...
            if out:return polish(restore(out,repl))
        except Exception as e:
            err=e;time.sleep(min(8,1.2*(attempt+1)))
    raise RuntimeError(f'translation failed after retries: {err}: {s[:160]}')
...
def translate_bundle(items):
    ...
    markers=['ZZSEG'+alpha_code(i)+'ZZ' for i in range(len(items)-1)]
    combined=''
    ...
    try:
        ...
        pattern='\s*(?:'+'|'.join(re.escape(m) for m in markers)+')\s*'
        pieces=re.split(pattern,out,flags=re.I)
        if len(pieces)==len(items):return [polish(x) for x in pieces]
    except Exception:
        pass
...
def translate_html(src):
    ...
    for part in parts:
        ...
        m1=re.match(r'^\s*',decoded);m2=re.search(r'\s*$',decoded)
        lead=m1.group(0);trail=m2.group(0);end=len(decoded)-len(trail) if trail else len(decoded)
        core=decoded[len(lead):end];k=norm(core)
...
strings=set()
for p in products:
    strings.add(norm(p['title']))
    collect_html(p.get('excerpt',''),strings);collect_html(p.get('content',''),strings)
# Manual titles do not need MT for their title text, but harmless if already in set.
fill_cache(strings)

out=[]
for p in products:
    src_title=norm(p['title'])
    en_title=MANUAL_TITLES.get(src_title,cache[src_title])
    en_title=polish(en_title)
    # More natural title-level cleanup.
    en_title=re.sub(r'^Selection\s+','',en_title,flags=re.I) if src_title.lower().startswith('lote puente robles') else en_title
    en_title=en_title[0].upper()+en_title[1:] if en_title else en_title
    row={'id':p['id'],'author_id':p['author_id'],'vendor':p['vendor'],'source_status':p['status'],'title':en_title,'slug':slugify(en_title),'excerpt':translate_html(p.get('excerpt','')),'content':translate_html(p.get('content',''))}
    out.append(row)
