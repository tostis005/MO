...
function mdo_staging_polish_allowed() {
    return ! is_admin()
        && mdo_staging_polish_is_english()
        && strpos( home_url( '/' ), 'dev.elmercadodeorigen.com' ) !== false;
}
...
add_action( 'template_redirect', function() {
    if ( ! mdo_staging_polish_allowed() ) { return; }
...
add_action( 'wp_footer', function() {
    if ( ! mdo_staging_polish_allowed() ) { return; }
    ?>
    <script id="mdo-staging-visible-english-polish">
    (function () {
      const exact = new Map([
      ...
      function keepWhitespace(original, replacement) {
      ...
      function translateText(value) {
        ...
        if (exact.has(key)) return keepWhitespace(value, exact.get(key));

        let out = value;
        out = out.replace(/\b(\d+) productos\b/gi, '$1 products');
        out = out.replace(/Paleta de acorn-fed 100% Iberian/gi, '100% Iberian acorn-fed shoulder ham');
        out = out.replace(/Jamón de acorn-fed 100% Iberian/gi, '100% Iberian acorn-fed ham');
        out = out.replace(/Paleta de acorn-fed 50% Iberian/gi, '50% Iberian acorn-fed shoulder ham');
        out = out.replace(/Jamón de acorn-fed 50% Iberian/gi, '50% Iberian acorn-fed ham');
        out = out.replace(/Paleta de free-range grain-fed 100% Iberian/gi, '100% Iberian free-range grain-fed shoulder ham');
        out = out.replace(/Jamón de free-range grain-fed 100% Iberian/gi, '100% Iberian free-range grain-fed ham');
        out = out.replace(/Paleta de bellota 100% Ibérica/gi, '100% Iberian acorn-fed shoulder ham');
        out = out.replace(/Jamón de bellota 100% Ibérico/gi, '100% Iberian acorn-fed ham');
        out = out.replace(/Paleta de bellota 50% Ibérica/gi, '50% Iberian acorn-fed shoulder ham');
        out = out.replace(/Jamón de bellota 50% Ibérico/gi, '50% Iberian acorn-fed ham');
        if (out.trim() === 'Jamón') out = keepWhitespace(out, 'Ham');
        if (out.trim() === 'Paleta') out = keepWhitespace(out, 'Shoulder ham');
        return out;
      }
      ...
      document.documentElement.setAttribute('data-mdo-visible-english', 'ready');
    })();
    </script>
    <?php
}, 1000 );
