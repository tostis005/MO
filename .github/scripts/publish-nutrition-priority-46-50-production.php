<?php
/** Publish bilingual nutrition priority articles 46-50 (Iberian loin nutrition). */
...
function emdo_4650_articles( string $dir ): array {
    $files=glob($dir.'/content-seeds/nutrition-priority-46-50-010275-v2.part*');
    ...
    if(hash('sha256',$encoded)!=='c21a7859ebb40c5de5c131a3409e93be8b6dd2e47b357b1802c223767747e27c'){ WP_CLI::error('Payload integrity check failed.'); }
    ...
    foreach($data as $a){
        foreach($required as $k){ if(!isset($a[$k]) || (is_string($a[$k]) && ''===trim($a[$k]))){ WP_CLI::error('Missing '.$k.' in '.($a['slug']??'unknown')); } }
...
function emdo_4650_assert_existing_matches(int $id,array $a,int $cat_id,string $es,string $en): void {
    $p=get_post($id); $slug=(string)$a['slug'];
    if(!$p instanceof WP_Post){ WP_CLI::error('Existing target could not be loaded: '.$slug.' id='.$id); }
    $m=array();
    if('publish'!==(string)$p->post_status){$m[]='status';}
    if(trim((string)$p->post_title)!==trim((string)$a['title'])){$m[]='title';}
    if((string)$p->post_name!==$slug){$m[]='slug';}
    if(trim((string)$p->post_excerpt)!==trim((string)$a['excerpt'])){$m[]='excerpt';}
    if(trim((string)$p->post_content)!==trim($es)){$m[]='content';}
...
$articles=emdo_4650_articles($seed_dir); $image_id=emdo_4650_image_id();
if($image_id<=0){ WP_CLI::error('Generic provisional image not found.'); }
$rows=array(); $errors=array();
foreach($articles as $a){
    $slug=(string)$a['slug'];
    $cat_id=emdo_4650_category_id($a); if($cat_id<=0){ WP_CLI::error('Blog category not found: '.$a['category_slug']); }
    $product_slugs=emdo_4650_product_slugs($a);
    $es=emdo_4650_render($a,$product_slugs,false); $en=emdo_4650_render($a,$product_slugs,true);
    ...
    $existing_id=emdo_4650_existing_post_id($slug); $adopted=false;
    if($existing_id>0){
        emdo_4650_assert_existing_matches($existing_id,$a,$cat_id,$es,$en);
        $id=$existing_id; $adopted=true; WP_CLI::log('Existing article matches expected payload; adopting safely: '.$slug.' id='.$id);
    }else{
        $r=wp_insert_post(wp_slash(array('post_type'=>'post','post_status'=>'publish','post_title'=>(string)$a['title'],'post_name'=>$slug,'post_excerpt'=>(string)$a['excerpt'],'post_content'=>$es,'post_category'=>array($cat_id),'comment_status'=>'closed','ping_status'=>'closed')),true);
        if(is_wp_error($r)||(int)$r<=0){ WP_CLI::error('Could not publish '.$slug); }
        $id=(int)$r; update_post_meta($id,'_emdo_batch_token',$batch_token);
        ...
        set_post_thumbnail($id,$image_id); emdo_4650_save_en($id,$a,$en);
    }
    $p=get_post($id);
