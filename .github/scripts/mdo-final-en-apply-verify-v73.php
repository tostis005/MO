...
function mdo_num_canon($x){$x=str_replace(',','.',(string)$x);if(strpos($x,'.')!==false){$x=rtrim(rtrim($x,'0'),'.');}if($x==='')$x='0';if(strpos($x,'.')===0)$x='0'.$x;return $x;}
function mdo_nums($s){$s=html_entity_decode(wp_strip_all_tags((string)$s),ENT_QUOTES|ENT_HTML5,'UTF-8');preg_match_all('/\d+(?:[.,]\d+)?/u',$s,$m);$o=[];foreach($m[0] as $x){$x=mdo_num_canon($x);$o[$x]=($o[$x]??0)+1;}ksort($o);return $o;}
...
foreach($payload['products'] as $row){
  ...
  if(isset($expected[(int)$row['author_id']])){if((int)$p->post_author!==(int)$row['author_id']||$row['vendor']!==$expected[(int)$row['author_id']]){$problems[]=['id'=>$id,'reason'=>'vendor'];continue;}}
  elseif(isset($hidalgo[$id])){if($row['vendor']!=='Hidalgo de la Jara'){$problems[]=['id'=>$id,'reason'=>'hidalgo_vendor'];continue;}}
...
if($problems){echo 'PRECHECK_BAD '.wp_json_encode(array_slice($problems,0,80),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT)."\n";fwrite(STDERR,'Precheck failed '.count($problems)." products\n");exit(22);}
$backup=['created'=>gmdate('c'),'site'=>get_option('siteurl'),'products'=>[]];foreach($validated as $row){$id=(int)$row['id'];$all=get_post_meta($id);$en=[];foreach($all as $k=>$vals){if(strpos($k,'_en_US_')===0)$en[$k]=$vals;}$backup['products'][]=['id'=>$id,'status'=>get_post_status($id),'meta'=>$en];}
$up=wp_upload_dir();$dir=trailingslashit($up['basedir']).'mdo-translation-backups';wp_mkdir_p($dir);$backup_path=trailingslashit($dir).'final-en-v73-'.gmdate('Ymd-His').'.json';file_put_contents($backup_path,wp_json_encode($backup,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
foreach($validated as $row){$id=(int)$row['id'];update_post_meta($id,'_en_US_post_title',(string)$row['title']);update_post_meta($id,'_en_US_post_content',(string)$row['content']);update_post_meta($id,'_en_US_post_excerpt',(string)$row['excerpt']);update_post_meta($id,'_en_US_post_name',(string)$row['slug']);if($row['vendor']==='Puente Robles'||$row['vendor']==='El Catedrático'){update_post_meta($id,'_en_US_published','0');update_post_meta($id,'_en_US_ready','1');}else{update_post_meta($id,'_en_US_published','1');delete_post_meta($id,'_en_US_ready');}}
wp_cache_flush();
$verify=[];$vb=[];foreach($validated as $row){$id=(int)$row['id'];$p=get_post($id);$vendor=$row['vendor'];$verify[$vendor]=($verify[$vendor]??0)+1;$title=(string)get_post_meta($id,'_en_US_post_title',true);$content=(string)get_post_meta($id,'_en_US_post_content',true);$excerpt=(string)get_post_meta($id,'_en_US_post_excerpt',true);$slug=(string)get_post_meta($id,'_en_US_post_name',true);$pub=(string)get_post_meta($id,'_en_US_published',true);$ready=(string)get_post_meta($id,'_en_US_ready',true);if($p->post_status!==$row['source_status']||$title!==$row['title']||$content!==$row['content']||$excerpt!==$row['excerpt']||sanitize_title($slug)!==sanitize_title($row['slug']))$vb[]=['id'=>$id,'reason'=>'write_mismatch'];if(($vendor==='Puente Robles'||$vendor==='El Catedrático')&&($pub!=='0'||$ready!=='1'))$vb[]=['id'=>$id,'reason'=>'prelaunch_flags','pub'=>$pub,'ready'=>$ready];if($vendor==='Hidalgo de la Jara'&&$pub!=='1')$vb[]=['id'=>$id,'reason'=>'hidalgo_not_published'];}
