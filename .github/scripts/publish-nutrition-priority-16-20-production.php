...
$seed_dir = getenv( 'EMDO_NUTRITION_1620_SEED_DIR' );
if ( ! is_string( $seed_dir ) || '' === trim( $seed_dir ) || ! is_dir( $seed_dir ) ) {
    WP_CLI::error( 'EMDO_NUTRITION_1620_SEED_DIR is missing or invalid.' );
}
$seed_dir = rtrim( $seed_dir, '/\\' );

function emdo_nutrition_1620_articles( string $seed_dir ): array {
    $encoded = '';
    for ( $part = 1; $part <= 4; $part++ ) {
        $file = $seed_dir . '/content-seeds/veg-nutrition-010270-' . $part . '.b64';
        if ( ! is_readable( $file ) ) { WP_CLI::error( 'Missing nutrition payload part ' . $part . '.' ); }
        $encoded .= trim( (string) file_get_contents( $file ) );
    }
    $compressed = base64_decode( $encoded, true );
    if ( false === $compressed ) { WP_CLI::error( 'Nutrition payload is not valid Base64.' ); }
    $json = gzdecode( $compressed );
    if ( false === $json ) { WP_CLI::error( 'Nutrition payload cannot be decompressed.' ); }
...
function emdo_nutrition_1620_category_id( array $article ): int {
...
function emdo_nutrition_1620_generic_image_id(): int {
    ...
    if ( ! empty( $attachments ) ) { return (int) $attachments[0]; }
...
function emdo_nutrition_1620_save_english( int $post_id, array $article, string $english_content ): void {
...
$articles = emdo_nutrition_1620_articles( $seed_dir );
$image_id = emdo_nutrition_1620_generic_image_id();
...
foreach ( $articles as $article ) {
    ...
    $category_id = emdo_nutrition_1620_category_id( $article );
    if ( $category_id <= 0 ) { WP_CLI::error( 'Blog category could not be resolved for: ' . $slug ); }
    $product_cat = emdo_nutrition_1620_product_cat( $article );
    if ( ! $product_cat instanceof WP_Term ) { WP_CLI::error( 'WooCommerce product category could not be resolved for: ' . $slug ); }

    $spanish_content = emdo_nutrition_1620_render_content( $article, $product_cat, false );
    $english_content = emdo_nutrition_1620_render_content( $article, $product_cat, true );
    ...
    $result = wp_insert_post( wp_slash(array(
        'post_type'=>'post','post_status'=>'publish','post_title'=>(string)$article['title'],
        'post_name'=>$slug,'post_excerpt'=>(string)$article['excerpt'],'post_content'=>$spanish_content,
        'post_category'=>array($category_id),'comment_status'=>'closed','ping_status'=>'closed',
    )), true );
    if ( is_wp_error($result) || (int)$result <= 0 ) { WP_CLI::error( 'Could not publish article: ' . $slug ); }

    $post_id=(int)$result;
    set_post_thumbnail($post_id,$image_id);
    emdo_nutrition_1620_save_english($post_id,$article,$english_content);

    $post=get_post($post_id);
    if ( ! $post instanceof WP_Post || 'publish' !== $post->post_status ) $errors[]='Post not published: '.$slug;
    if ( trim((string)$post->post_title) !== trim((string)$article['title']) ) $errors[]='Spanish title mismatch: '.$slug;
    if ( ! has_category($category_id,$post_id) ) $errors[]='Category missing: '.$slug;
    if ( (int)get_post_thumbnail_id($post_id) <= 0 ) $errors[]='Featured image missing: '.$slug;
    if ( false === strpos((string)$post->post_content,'[products category="'.$product_cat->slug.'"') ) $errors[]='Related products missing: '.$slug;
    ...
    foreach ( array(
        ...
        if ( trim((string)get_post_meta($post_id,$meta_key,true)) !== trim($expected) ) $errors[]='English meta mismatch '.$meta_key.': '.$slug;
    }
    if ( '1' !== (string)get_post_meta($post_id,'_en_US_published',true) ) $errors[]='English publication flag missing: '.$slug;

    $rows[] = array(
        'id'=>$post_id,'slug'=>$slug,'en_slug'=>(string)$article['en_slug'],
...
if ( ! $out['verified'] ) WP_CLI::error( 'Nutrition batch 16-20 publication verification failed.' );
