...
php -l .github/scripts/publish-nutrition-priority-31-35-production.php
python3 - <<'PY'
import base64,gzip,json
from pathlib import Path
parts=sorted(Path('elmercadodeorigen-child/inc/content-seeds').glob('nutrition-priority-31-35-010273.part*'))
...
expected=['legumbres','legumbres','jamones-y-paletas','jamones-y-paletas','jamones-y-paletas']
for i,a in enumerate(data):
    ...
    if not a['product_cat_slugs']:
        raise SystemExit('Product categories missing: '+a['slug'])
Path('/tmp/nutrition-3135.json').write_text(json.dumps(data,ensure_ascii=False),encoding='utf-8')
...
mapfile -t TARGET_SLUGS < <(jq -r '.[].slug' /tmp/nutrition-3135.json)
[ "${#TARGET_SLUGS[@]}" -eq 5 ]
...
CRITICAL_URLS=(
 ...
 'https://www.elmercadodeorigen.com/category/jamones-y-paletas/'
)
health_check(){
  local phase="$1"; shift
  for url in "$@"; do
...
REMOTE="/tmp/emdo-nutrition-3135-mixed-${GITHUB_RUN_ID}"
...
sshpass -e scp $SCP .github/scripts/publish-nutrition-priority-31-35-production.php "$STAGING_USER@$STAGING_HOST:$REMOTE/"
sshpass -e scp $SCP elmercadodeorigen-child/inc/content-seeds/nutrition-priority-31-35-010273.part* "$STAGING_USER@$STAGING_HOST:$REMOTE/content-seeds/"

ROLLBACK_ACTIVE=1
rollback_only_new_posts(){
  rc=$?; set +e
  if [ "${ROLLBACK_ACTIVE:-0}" = 1 ]; then
    echo 'Verification failed: rolling back only new batch 31-35 posts.' >&2
    for slug in "${TARGET_SLUGS[@]}"; do
      ids="$(sshpass -e ssh $SSH "$STAGING_USER@$STAGING_HOST" "export PATH='$(dirname "$PHP")':\$PATH; '$PHP' '$WP' post list --path='$PROD' --allow-root --post_type=post --post_status=any --name='$slug' --format=ids 2>/dev/null || true")"
...
trap - ERR; set +e
sshpass -e ssh $SSH "$STAGING_USER@$STAGING_HOST" "export EMDO_NUTRITION_3135_SEED_DIR='$REMOTE'; export PATH='$(dirname "$PHP")':\$PATH; '$PHP' -d memory_limit=768M '$WP' eval-file '$REMOTE/publish-nutrition-priority-31-35-production.php' --path='$PROD' --allow-root" >/tmp/nutrition-3135.raw 2>/tmp/nutrition-3135.err
PUBLISH_RC=$?; set -e; trap rollback_only_new_posts ERR
[ ! -s /tmp/nutrition-3135.err ] || cat /tmp/nutrition-3135.err >&2
[ ! -s /tmp/nutrition-3135.raw ] || cat /tmp/nutrition-3135.raw
[ "$PUBLISH_RC" -eq 0 ]
JSON_LINE="$(grep -E '^\{.*\}$' /tmp/nutrition-3135.raw|tail -n1 || true)"; [ -n "$JSON_LINE" ]
printf '%s\n' "$JSON_LINE" >/tmp/nutrition-3135-result.json
jq -e '.verified==true and .count==5 and (.errors|length)==0 and (.posts|length)==5' /tmp/nutrition-3135-result.json >/dev/null
...
done < <(jq -r '.posts[]|[.slug,.en_slug,.title,.en_title,.permalink,.en_permalink,.category_slug]|@tsv' /tmp/nutrition-3135-result.json)

curl -L -sS --retry 3 -A "$UA" "https://www.elmercadodeorigen.com/blog/?emdo_verify=${GITHUB_RUN_ID}" -o /tmp/blog.html
while read -r slug; do grep -Fq "$slug" /tmp/blog.html; done < <(jq -r '.posts[].slug' /tmp/nutrition-3135-result.json)
health_check post "${CRITICAL_URLS[@]}"
ROLLBACK_ACTIVE=0; trap - ERR
sshpass -e ssh $SSH "$STAGING_USER@$STAGING_HOST" "rm -rf '$REMOTE'"
echo 'SAFE_PUBLICATION_OK: batch 31-35 nutrition articles published and verified.'
