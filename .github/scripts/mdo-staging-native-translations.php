...
if ( strpos( $siteurl, 'https://dev.elmercadodeorigen.com' ) !== 0 ) { fwrite( STDERR, "Refusing non-staging site: {$siteurl}\n" ); exit( 2 ); }
...
if ( ! is_object( $query ) || ! method_exists( $query, 'check_table' ) || ! method_exists( $query, 'update_strings' ) ) { exit( 6 ); }
...
global $wpdb;
if ( empty( $table ) || $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) !== $table ) { exit( 7 ); }
...
/*
 * Gettext strings are already native TranslatePress rows. On this Linux host the
 * physical table is lower-case (wp_trp_gettext_en_us); use that exact native table
 * and alter only translated/status, preserving domain, original_id and plural data.
 */
