...
async function go(page,path,delay=650){
  const u=new URL(path,BASE);u.searchParams.set('qa-010108',Date.now());
...
async function homeCheck(page,w,h){
  ...
  await page.screenshot({path:`qa/user-request-010108-home-${w}.png`,fullPage:true});
}
...
async function cartCheck(page,id,w=390,h=844){
  ...
  await page.screenshot({path:`qa/user-request-010108-cart-${w}.png`,fullPage:true});
}
...
(async()=>{
  ...
  fs.writeFileSync('qa/user-request-010108-check.json',JSON.stringify({failures,checks},null,2));if(failures.length){console.error('USER_REQUEST_010108_FAIL '+JSON.stringify(failures));process.exitCode=2}else console.log('USER_REQUEST_010108_OK');
})();
