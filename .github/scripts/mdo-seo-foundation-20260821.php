...
function emdo_seo_title_for_request( string $current ): string {
    ...
    if ( function_exists( 'is_product_category' ) && is_product_category() ) {
        $name = emdo_seo_clean_term_name( emdo_seo_term_name() );
        if ( $name !== '' ) return sprintf( 'Comprar %s online | El Mercado de Origen', $name );
    }
    return $current;
}
...
function emdo_seo_description_for_request( string $current ): string {
    ...
    if ( function_exists( 'is_product_category' ) && is_product_category() ) {
        $name = emdo_seo_clean_term_name( emdo_seo_term_name() );
        if ( $name !== '' ) return sprintf( 'Compra %s online directamente a productores seleccionados. Conoce su origen, quién los elabora y recibe tu pedido en casa.', mb_strtolower( $name, 'UTF-8' ) );
    }
    return $current;
}
...
add_filter( 'get_post_metadata', static function ( $value, $object_id, $meta_key, $single ) {
    if (
        emdo_seo_is_en()
        && ( emdo_seo_path() === '/en/' || emdo_seo_path() === '/en' )
        && $meta_key === '_en_US_post_excerpt'
        && (int) $object_id === (int) get_option( 'page_on_front' )
    ) {
        $text = 'Discover selected Spanish food chosen for its origin, quality and producer. Buy directly from trusted producers at El Mercado de Origen.';
...
add_filter( 'aioseo_canonical_url', static function ( $url ) {
    $path = emdo_seo_path();
    if ( $path === '/en/' || $path === '/en' ) return emdo_seo_root() . '/en/';
    return $url;
}, PHP_INT_MAX );

add_filter( 'get_the_excerpt', static function ( $excerpt, $post ) {
    if ( ! emdo_seo_is_en() || ! preg_match( '#^/en/(?:journal|blog)/?$#i', emdo_seo_path() ) ) return $excerpt;
    ...
    if ( $translated === '' ) $translated = (string) get_post_meta( $post_id, '_en_US_post_content', true );
